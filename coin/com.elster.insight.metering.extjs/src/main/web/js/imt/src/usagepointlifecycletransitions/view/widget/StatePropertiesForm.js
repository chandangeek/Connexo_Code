Ext.define('Imt.usagepointlifecycletransitions.view.widget.StatePropertiesForm', {
    ...
    createFieldForItem: function (item) {
        ...
        if (!Ext.isEmpty(item.radioGroup)) {
            ...
            Ext.each(_.sortBy(item.radioGroup, function (itemRadio) {
                return itemRadio.get('name')
            }), function (radioBtn) {
        ...
        if (item.isRequired) {
            switch (me.itemStack) {
                case 'actions':
                    additionalMessage = Uni.I18n.translate('usagePointLifeCycleTransitions.add.partOfMandatoryAction', 'IMT', 'This is a part of the mandatory auto actions.');
                    break;
                case 'checks':
                    additionalMessage = Uni.I18n.translate('usagePointLifeCycleTransitions.add.partOfMandatoryChecks', 'IMT', 'This is a part of the mandatory checks.');
                    break;
            }
    ...
    initComponent: function () {
        ...
        if (me.propertiesStore.count() > 0) {
            ...
            me.items = [
                {
                    ...
                    items: [
                        {
                            text: Uni.I18n.translate('usagePointLifeCycleTransitions.add.showOptional', 'IMT', 'Show optional only'),
                        ...
                        {
                            text: Uni.I18n.translate('usagePointLifeCycleTransitions.add.showAll', 'IMT', 'Show all'),
                            itemId: 'showAllBtn',
                            scope: me,
            ...
            Ext.iterate(groupedProperties, function (key, items) {
                ...
                Ext.each(_.sortBy(items, function (item) {
                    return item.name;
                }), function (item) {
                    if (item.isRequired) {
                        isAllOptional = false;
            ...
            switch (me.itemStack) {
                case 'actions':
                    noItemsMessage = Uni.I18n.translate('usagePointLifeCycleTransitions.add.noMandatoryAction', 'IMT', 'No auto actions are available.');
                    break;
                case 'checks':
                    noItemsMessage = Uni.I18n.translate('usagePointLifeCycleTransitions.add.noMandatoryChecks', 'IMT', 'No pretransition checks are available.');
                    break;
            }
