...
function HuginContext() {
    ...
    this.resume = function () {
        ...
        while (true) {
            ...
            if (task == null) {	//All tasks were finished.
                context.bExecuting = false;
                return true;
            }
            ...
            if (context.bError) {	//Failed. Stop executing.
                context.bExecuting = false;
            ...
            if (context.bCallBack && !context.bCallBackReady) {	//Callback function not finished. Give the resume right to it.
                context.bExecuting = false;
                return;
            }
            ...
            if (context.needBreathe()) {	//Give GUI a chance to process messages.
                context.bExecuting = false;
                updateResultView();
                setTimeout("context.resume();", 1);
                return;
...
function HuginHunter() {
    ...
    this.getTopicInfo = function (a_Context, a_this) {	// the database will fill the query result in the same mapRankedTopics
