...
function initializeLayout() {/* This function makes changes to the layout, including widgets. Called through RH event handler. */
    ...
    readSetting(lastVisitedTopic, changeTopicLink, false, false);
    /* Check for the latest visited topic. */
    ...
    desktopSidebar.toggleElement.click(function () {
        desktopSidebar.toggle()
    });
    readSetting(desktopSidebar.lastSideBarWidthSetting, setLastSidebarWidth, false, false);
    /* Load sidebar width setting */
    readSetting(sidebarCSHRunCheck, setSidebarCSHRunCheck, false, false);
    /* Begin the CSH check for the sidebar. Only show/hide the sidebar in CSH mode the first time the help is opened. */
    $(window).resize(function () {
    ...
    setCheckBoxes();
    /* Change checkboxes into On/Off button */

    /* Set tab group hover */
    $("ul.wTabGroup li").hover(
        function () {
            tabButtonEvent($("ul.wTabGroup"))
        },
        function () {
            tabButtonEvent($("ul.wTabGroup"))
        })
    ...
    setTimeout(function () {
        tabButtonEvent($("ul.wTabGroup"));
        /* Set images in tabs on sidebar */
    }, 10);
    /* Set timeout because IE doesn't show the correct image directly. */

    var advancedSearch = new elementToggle($("div.cSearchSettingsToggle a"), $("div.wSearchResultSettings"));
    advancedSearch.setLinkState();
    advancedSearch.link.click(function () {
...
var elementToggle = function (link, target) {
    ...
    this.state = function () {
        return (this.target.is(":visible")) ? true : false;
    };
...
var resizableSidebar = function (sidebar, reverseWidthElementName, baselineElement) {
    ...
    this.setSidebarWidth = function (width) {
        this.sidebar.width(width);
    };
    this.reverse = reverseWidthElementName;
    this.reverseElement = $(reverseWidthElementName);
    this.reverseWidth = function () {
        return this.reverseElement.width();
    };
    this.setReverseElementWidth = function (width) {
        this.reverseElement.width(width);
    };
    this.baseline = baselineElement;
    this.baselineWidth = function () {
        return this.baseline.width();
    };
    this.maximumWidth = null
    this.minimumWidth = null;
    this.setMaximumWidth = function (maximum) {
        this.maximumWidth = maximum
    };
    this.resetMaximumWidth = function () {
        this.maximumWidth = this.baselineWidth() * 0.75;
    };
    this.setMinimumWidth = function (minimum) {
    ...
    this.toggleElementWidth = function () {
        return this.toggleElement.width();
    };
    ...
    this.setToggleOpeningState = function () {
        this.toggleElement.addClass(this.toggleElementHiddenStyle);
        this.toggleElement.removeClass(this.toggleElementVisibleStyle);
    };
    this.setToggleClosingState = function () {
        this.toggleElement.addClass(this.toggleElementVisibleStyle);
        this.toggleElement.removeClass(this.toggleElementHiddenStyle);
    };
    ...
    this.saveSidebarWidth = function () {
        saveSetting(this.lastSideBarWidthSetting, this.sidebarWidth(), false);
    };
...
$.ui.plugin.add("resizable", "alsoResizeReverseWidth", {
    ...
    start: function () {
        ...
        if (typeof(o.alsoResizeReverseWidth) === "object" && !o.alsoResizeReverseWidth.parentNode) {
            if (o.alsoResizeReverseWidth.length) {
                o.alsoResizeReverseWidth = o.alsoResizeReverseWidth[0];
                _store(o.alsoResizeReverseWidth);
            }
            else {
                $.each(o.alsoResizeReverseWidth, function (exp) {
                    _store(exp);
                });
            }
        } else {
    ...
    resize: function (event, ui) {
        ...
        if (typeof(o.alsoResizeReverseWidth) === "object" && !o.alsoResizeReverseWidth.nodeType) {
            $.each(o.alsoResizeReverseWidth, function (exp, c) {
                _alsoResizeReverseWidth(exp, c);
            });
        } else {
