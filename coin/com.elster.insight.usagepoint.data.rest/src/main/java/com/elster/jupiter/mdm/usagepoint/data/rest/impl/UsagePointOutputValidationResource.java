...
public class UsagePointOutputValidationResource {
    ...
    public PagedInfoList getUsagePointChannelValidationConfiguration(@PathParam("name") String name, @PathParam("purposeId") long contractId,
        ...
        List<ChannelValidationRuleInfo> infos = usagePointConfigurationService.getValidationRuleSets(deliverable.getMetrologyContract())
                ...
                .collect(Collectors.toMap(ChannelValidationRuleInfo::getKey, Function.identity(), ChannelValidationRuleInfo::chooseEffectiveOne)).values().stream()
                .sorted()
    ...
    @Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
    @RolesAllowed({Privileges.Constants.VIEW_ANY_USAGEPOINT, Privileges.Constants.VIEW_OWN_USAGEPOINT})
    public ChannelValidationRuleInfo getUsagePointChannelValidationRuleById(@PathParam("name") String name, @PathParam("purposeId") long contractId, @PathParam("outputId") long outputId,
                                                                            @PathParam("ruleId") long ruleId, @BeanParam JsonQueryParameters queryParameters) {
        UsagePoint usagePoint = resourceHelper.findUsagePointByNameOrThrowException(name);
        ReadingTypeDeliverable deliverable = findReadingTypeDeliverableOrThrowException(usagePoint, contractId, outputId);
        ReadingType readingType = deliverable.getReadingType();
        ValidationRule validationRule = resourceHelper.findValidationRuleOrThrowException(ruleId);
        UsagePointValidation usagePointValidation = usagePointDataModelService.forValidation(usagePoint);
        Optional<? extends ChannelValidationRuleOverriddenProperties> overriddenProperties = usagePointValidation.findOverriddenProperties(validationRule, readingType);
        if (overriddenProperties.isPresent()) {
            return channelValidationRuleInfoFactory.createInfoForRule(validationRule, readingType, overriddenProperties.get());
        } else {
            return channelValidationRuleInfoFactory.createInfoForRule(validationRule, readingType);
        }
    }
    ...
    @Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
    @RolesAllowed({Privileges.Constants.VIEW_ANY_USAGEPOINT, Privileges.Constants.VIEW_OWN_USAGEPOINT})
    public Response editChannelValidationRuleOverriddenProperties(@PathParam("name") String name, @PathParam("purposeId") long contractId, @PathParam("outputId") long outputId,
    ...
    @Produces(MediaType.APPLICATION_JSON + "; charset=UTF-8")
    @RolesAllowed({Privileges.Constants.VIEW_ANY_USAGEPOINT, Privileges.Constants.VIEW_OWN_USAGEPOINT})
    public Response restoreChannelValidationRuleProperties(@PathParam("name") String name, @PathParam("purposeId") long contractId, @PathParam("outputId") long outputId,
                                                           @PathParam("ruleId") long ruleId, ChannelValidationRuleInfo channelValidationRuleInfo) {
        channelValidationRuleInfo.ruleId = ruleId;
        UsagePoint usagePoint = resourceHelper.findUsagePointByNameOrThrowException(name);
        ReadingType readingType = findReadingTypeDeliverableOrThrowException(usagePoint, contractId, outputId).getReadingType();
