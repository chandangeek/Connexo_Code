...
@Component(
        name = "com.elster.jupiter.mdm.demo.customtask.impl.DemoCustomTaskFactory",
...
public class DemoCustomTaskFactory implements CustomTaskFactory {
    ...
    public static final String NAME = "MDMDemoCustomTask";
    public static final String SUBSCRIBER_NAME = "MDMDemoCustomTask";
    public static final String DESTINATION_NAME = "MDMDemoCustomTask";
    static final String SUBSCRIBER_DISPLAY_NAME = "Handle MDM custom task";
    static final String DISPLAY_NAME = "Demo custom task for MDM";
    ...
    public List<CustomTaskAction> getActionsForUser(User user, String application) {
        ...
        user.getPrivileges(application).stream()
                .filter(privilege -> privilege.getName().compareToIgnoreCase(Privileges.Constants.ADMINISTRATE_MDM_DEMO_CUSTOM_TASK) == 0)
                .forEach(privilege -> customTaskActions.addAll(Arrays.asList(CustomTaskAction.ADMINISTRATE, CustomTaskAction.EDIT)));

        user.getPrivileges(application).stream()
                .filter(privilege -> privilege.getName().compareToIgnoreCase(Privileges.Constants.VIEW_MDM_DEMO_CUSTOM_TASK) == 0)
        ...
        user.getPrivileges(application).stream()
                .filter(privilege -> privilege.getName().compareToIgnoreCase(Privileges.Constants.RUN_MDM_DEMO_CUSTOM_TASK) == 0)
    ...
    private PropertySpec getDataSelectorProperty() {
       List<String> allGroups = meteringGroupsService.findUsagePointGroups()
                .stream()
                .map(endDeviceGroup -> new String(endDeviceGroup.getName()))
                .collect(Collectors.toList());

        allGroups.sort(Comparator.comparing(a -> a, String.CASE_INSENSITIVE_ORDER));
