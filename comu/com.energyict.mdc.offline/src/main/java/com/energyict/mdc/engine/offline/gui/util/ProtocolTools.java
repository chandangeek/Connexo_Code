...
public final class ProtocolTools {
    ...
    /**
     * Convert a given byte array into an long
     */
    public static long getLongFromBytes(byte[] bytes, int offset, int length) {
        byte[] byteArray = getSubArray(bytes, offset, offset + length);
        long value = 0;
        for (int i = 0; i < byteArray.length; i++) {
            long longByte = byteArray[i] & 0x0FF;
            value += longByte << ((byteArray.length - (i + 1)) * 8);
        }
        return value;
    }
    ...
    public static List<IntervalData> mergeDuplicateIntervals(List<IntervalData> intervals) {
        ...
        for (IntervalData id2compare : intervals) {
            ...
            if (!allreadyProcessed) {
                ...
                Number[] values = new Number[id2compare.getValueCount()];
                IntervalData md = new IntervalData(id2compare.getEndTime());
                for (IntervalData intervalData : toAdd) {
                    for (int i = 0; i < values.length; i++) {
                        if (values[i] == null) {
                            values[i] = intervalData.get(i);
                        } else {
                            values[i] = NumberTools.add(values[i], intervalData.get(i));
                        }
                    }
                }
                md.addValues(Arrays.asList(values));
    ...
    public static String getAsciiFromBytes(byte[] b) {
        return getAsciiFromBytes(b, 0, b != null ? b.length : 0, '.');
    }

    /**
     * This method converts a byte array - taking into account the given length -
     * to a readable string. Unprintable characters are replaced by a '.'
     */
    public static String getAsciiFromBytes(byte[] b, int length) {
        return getAsciiFromBytes(b, 0, length, '.');
    }

    /**
     * This method converts a byte array - taking into account the given offset and length -
     * to a readable string. Unprintable characters are replaced by a '.'
     */
    public static String getAsciiFromBytes(byte[] b, int offset, int length) {
        return getAsciiFromBytes(b, offset, length, '.');
    }

    /**
     * This method converts a byte array to a readable string. Unprintable
     * characters are replaced by the chosen character
     */
    public static String getAsciiFromBytes(byte[] b, char character) {
        return getAsciiFromBytes(b, 0, b != null ? b.length : 0, character);
    }

    /**
     * This method converts a byte array - taking into account the given offset and length -
     * to a readable string. Unprintable characters are replaced by the parameter character
     */
    public static String getAsciiFromBytes(byte[] b, int offset, int length, char character) {
        if (b != null) {
    ...
    public static void printArray(byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            String s = String.join(":", Integer.toString(i), Byte.toString(bytes[i]), " ");
            System.out.print(s);
        }
