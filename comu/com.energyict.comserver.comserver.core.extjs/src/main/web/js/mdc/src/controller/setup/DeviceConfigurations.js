Ext.define('Mdc.controller.setup.DeviceConfigurations', {
    ...
    deleteTheDeviceConfiguration: function (deviceConfigurationToDelete) {
        ...
        Ext.create('Uni.view.window.Confirmation').show({
            ...
            title: Uni.I18n.translate('general.remove', 'MDC', "Remove '{0}'?",[Ext.String.htmlEncode(deviceConfigurationToDelete.get('name'))]),
            config: {
            ...
            success: function (deviceConfiguration) {
                ...
                Ext.ModelManager.getModel('Mdc.model.DeviceType').load(deviceTypeId, {
                    success: function (deviceType) {
                        me.getApplication().fireEvent('loadDeviceType', deviceType);
                        widget.down('form').loadRecord(deviceConfiguration);
                        widget.down('#deviceConfigurationEditCreateTitle').setTitle(Uni.I18n.translate('general.edit', 'MDC', "Edit '{0}'",[deviceConfiguration.get('name')]));
                        ...
                        success: function (deviceType) {
                            ...
                            deviceConfigModel.load(deviceConfigurationId, {
                                success: function (deviceConfiguration) {
                                    ...
                                    widget.down('#editLogbookPanel').setTitle(Uni.I18n.translate('general.edit', 'MDC', "Edit '{0}'",[form.down('[name=name]').getValue()]));
                                    widget.setLoading(false);
                                }
                            });
                        }
