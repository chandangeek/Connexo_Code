/*
 * Copyright (c) 2017 by Honeywell International Inc. All Rights Reserved
 */

Ext.define('Mdc.view.setup.comserver.OfflineComServersGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.offlineComServersGrid',
    store: 'Mdc.store.OfflineComServers',
    overflowY: 'auto',
    itemId: 'offlinecomservergrid',

    requires: [
        'Mdc.store.OfflineComServers',
        'Uni.view.toolbar.PagingTop',
        'Uni.view.toolbar.PagingBottom',
        'Uni.grid.column.Action',
        'Mdc.view.setup.comserver.ActionMenu',
        'Mdc.store.OfflineComServers'
    ],

    columns: [
        {
            header: Uni.I18n.translate('general.offlineComServer', 'MDC', 'Mobile communication server'),
            flex: 5,
            dataIndex: 'name',
            renderer: function (value, b, record) {
                return '<a href="#/administration/offlinecomservers/' + record.get('id') + '">' + Ext.String.htmlEncode(value) + '</a>';
            }
        },
        {
            header: Uni.I18n.translate('general.type', 'MDC', 'Type'),
            dataIndex: 'displayComServerType',
            flex: 2
        },
        {
            header: Uni.I18n.translate('general.status', 'MDC', 'Status'),
            dataIndex: 'active',
            flex: 2,
            renderer: function (value) {
                if (value === true) {
                    return Uni.I18n.translate('general.active', 'MDC', 'Active');
                } else {
                    return Uni.I18n.translate('general.inactive', 'MDC', 'Inactive');
                }
            }
        },
        {
            itemId: 'actionColumn',
            xtype: 'uni-actioncolumn',
            width: 120,
            menu: {
                xtype: 'comserver-actionmenu',
                itemId: 'offlineComServerViewMenu'
            }
        }
    ],

    dockedItems: [
        {
            xtype: 'pagingtoolbartop',
            store: 'Mdc.store.OfflineComServers',
            dock: 'top',
            displayMsg: Uni.I18n.translate('comserver.displayMsg', 'MDC', '{0} - {1} of {2} communication servers'),
            displayMoreMsg: Uni.I18n.translate('comserver.displayMoreMsg', 'MDC', '{0} - {1} of more than {2} communication servers'),
            items: [
                {
                    xtype: 'button',
                    itemId: 'btn-add-offline-communication-server',
                    text: Uni.I18n.translate('comServer.addOffline', 'MDC', 'Add mobile communication server'),
                    privileges: Mdc.privileges.Communication.admin,
                    href: '#/administration/offlinecomservers/add/offline'
                }
            ]
        },
        {
            xtype: 'pagingtoolbarbottom',
            itemsPerPageMsg: Uni.I18n.translate('comserver.itemsPerPageMsg', 'MDC', 'Communication servers per page'),
            store: 'Mdc.store.OfflineComServers',
            dock: 'bottom'
        }
    ]
});