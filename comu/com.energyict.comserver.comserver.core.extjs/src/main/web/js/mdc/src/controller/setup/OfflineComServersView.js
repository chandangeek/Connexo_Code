...
Ext.define('Mdc.controller.setup.OfflineComServersView', {
    ...
    views: [
        'Mdc.view.setup.comserver.OfflineComServersSetup',
        'Mdc.store.LogLevels',
    ...
    refs: [
        {
            ref: 'offlineComServersView',
            selector: 'offlineComServersSetup'
        },
        {
        ...
        {
            ref: 'offlineComServerPreview',
            selector: 'offlineComServersSetup offlineComServerPreview'
        },
        {
            ref: 'previewActionMenu',
            selector: 'offlineComServerPreview #offlineComServerViewMenu'
        }
    ],
    ...
    init: function () {
        var me = this;
        this.control({
            'offlineComServersGrid': {
                select: this.showOfflineComServerPreview
            },
            '#offlineComServerViewMenu': {
    ...
    chooseAction: function (menu, item) {
        var me = this,
            grid = me.getOfflineComServerGrid(),
        ...
        if (grid) {
            ...
            form = this.getOfflineComServerPreview().down('form');
        } else {
            record = menu.record;
            form = this.getOfflineComServerOverviewForm();
        }
    ...
    showOfflineComServerPreview: function (selectionModel, record) {
        var me = this,
            itemPanel = this.getOfflineComServerPreview(),
    ...
    fillPreviewForm: function (form, record) {
        var comServerStore = this.getStore('Mdc.store.ComServers'),
    ...
    openComserverMonitor: function (record) {
        if(record.get('serverMonitorUrl') != '') {
    ...
    deleteComserver: function (record) {
        var me = this,
            page = me.getOfflineComServersView(),
            gridToolbarTop = me.getOfflineComServerGrid().down('pagingtoolbartop');
