...
public class DeviceShipmentImportProcessor extends AbstractDeviceDataFileImportProcessor<DeviceShipmentImportRecord> {
    ...
    public void process(DeviceShipmentImportRecord data, FileImportLogger logger) throws ProcessorException {
        ...
        if (!is(data.getBatch()).emptyOrOnlyWhiteSpace()) {
            device = getContext().getDeviceService().newDevice(deviceConfiguration, data.getDeviceIdentifier(), data.getBatch(), Instant.now());
        } else {
            device = getContext().getDeviceService().newDevice(deviceConfiguration, data.getDeviceIdentifier(), data.getDeviceIdentifier(), Instant.now());
        }
