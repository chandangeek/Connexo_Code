...
public class AM130RegisterFactory implements DeviceRegisterSupport {
    ...
    protected List<CollectedRegister> readSubSetOfRegisters(List<OfflineRegister> registers) {
        ...
        ComposedCosemObject composedCosemObject = new ComposedCosemObject(getMeterProtocol().getDlmsSession(), getMeterProtocol().getDlmsProperties().isBulkRequest(), dlmsAttributes);
    ...
    protected Boolean addComposedObjectToComposedRegisterMap(Map<ObisCode, ComposedObject> composedObjectMap, List<DLMSAttribute> dlmsAttributes, OfflineRegister register) {
        ...
        if (isMBusValueChannel(register.getObisCode())) {
            obisCode = getMeterProtocol().getPhysicalAddressCorrectedObisCode(obisCode, register.getDeviceSerialNumber());
        }
    ...
    protected CollectedRegister createCollectedRegisterFor(OfflineRegister offlineRegister, Map<ObisCode, ComposedObject> composedObjectMap, ComposedCosemObject composedCosemObject) {
        ...
        if (isMBusValueChannel(offlineRegister.getObisCode())) {
            obisCode = getMeterProtocol().getPhysicalAddressCorrectedObisCode(obisCode, offlineRegister.getDeviceSerialNumber());
        }
                ...
                if (composedObject instanceof ComposedRegister) {
                    ...
                    if (composedRegister.getRegisterCaptureTime() != null) {
                        ...
                        if (dlmsDateTime.getDeviation() != configuredTimeZoneOffset) {
                            timeZoneIssue = getMeterProtocol().getIssueService().newWarning(offlineRegister.getObisCode(), MessageSeeds.REGISTER_NOT_SUPPORTED, offlineRegister.getObisCode(),
    ...
    protected RegisterValue getRegisterValueForAlarms(OfflineRegister offlineRegister, AbstractDataType dataValue) {
        ...
        if (offlineRegister.getObisCode().equals(ObisCode.fromString(ALARM_REGISTER1)) || offlineRegister.getObisCode().equals(ObisCode.fromString(ERROR_REGISTER))) {
            AlarmBitsRegister alarmBitsRegister = new AlarmBitsRegister(offlineRegister.getObisCode(), (Unsigned32) dataValue);
    ...
    protected List<CollectedRegister> filterOutAllInvalidRegistersFromList(List<OfflineRegister> offlineRegisters) {
        ...
        while (it.hasNext()) {
            OfflineRegister register = it.next();
            if (getMeterProtocol().getPhysicalAddressFromSerialNumber(register.getDeviceSerialNumber()) == -1) {
                invalidRegisters.add(createFailureCollectedRegister(register, ResultType.InCompatible, "Register " + register + " is not supported because MbusDevice " + register.getDeviceSerialNumber() + " is not installed on the physical device."));
                it.remove();
            }
        }
    ...
    protected CollectedRegister createFailureCollectedRegister(OfflineRegister register, ResultType resultType, Object... errorMessage) {
        CollectedRegister collectedRegister = getMeterProtocol().getCollectedDataFactory().createDefaultCollectedRegister(getRegisterIdentifier(register), register.getReadingType());
        if (resultType == ResultType.InCompatible) {
            collectedRegister.setFailureInformation(ResultType.InCompatible, getMeterProtocol().getIssueService()
                    .newWarning(register.getObisCode(), MessageSeeds.REGISTER_NOT_SUPPORTED, register.getObisCode(), errorMessage[0]));
        } else {
            if (errorMessage.length == 0) {
                collectedRegister.setFailureInformation(ResultType.NotSupported, getMeterProtocol().getIssueService()
                        .newWarning(register.getObisCode(), MessageSeeds.REGISTER_NOT_SUPPORTED, register.getObisCode()));
            } else {
                collectedRegister.setFailureInformation(ResultType.NotSupported, getMeterProtocol().getIssueService()
                        .newWarning(register.getObisCode(), MessageSeeds.REGISTER_NOT_SUPPORTED, register.getObisCode(), errorMessage[0]));
            }
    ...
    protected CollectedRegister createCollectedRegister(RegisterValue registerValue, OfflineRegister offlineRegister) {
        CollectedRegister deviceRegister = this.getMeterProtocol()
