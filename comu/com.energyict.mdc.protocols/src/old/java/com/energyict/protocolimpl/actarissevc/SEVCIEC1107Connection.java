...
public class SEVCIEC1107Connection {
     ...
     */
    public SEVCIEC1107Connection(InputStream inputStream, OutputStream outputStream, int iTimeout, int iMaxRetries, int forcedDelay) throws SEVCIEC1107ConnectionException {
    ...
    private void sendBreak() throws SEVCIEC1107ConnectionException {
        try {
            byte[] buffer = {(byte)SOH,(byte)0x42,(byte)0x30,(byte)ETX,(byte)0x21,(byte)0x11};
            sendRawData(buffer);
            return;
        }
    ...
    private void authenticate(String strPass) throws NestedIOException,SEVCIEC1107ConnectionException {
        ...
        for (int i=0;i<strPass.getBytes().length;i++) txbuffer[i] = strPass.getBytes()[i];
        byte[] crc=calcCRC(txbuffer);
        txbuffer[txbuffer.length-1] = (byte)(crc[0]);
