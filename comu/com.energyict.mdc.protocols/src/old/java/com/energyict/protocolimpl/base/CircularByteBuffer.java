 ...
 * and a PipedOutputStream. This class should be thread safe.
 */
public class CircularByteBuffer {
     ...
     * <p/>
     * Write methods may throw a IOException if the buffer is not large enough.
     * A large enough buffer size must be chosen so that this does not happen or
     * the caller must be prepared to catch the exception and try again once
     * part of the buffer has been consumed.
     *
    ...
    /**
     * Retrieve a InputStream that can be used to empty this buffer.<p/>
    ...
    /**
     * Get number of bytes that are available to be read.<p/>
    ...
    /**
     * Get the number of bytes this buffer has free for writing. <p/>
     * Note that the number of bytes available plus the number of bytes free may not add up to the
     * capacity of this buffer, as the buffer may reserve some space for other purposes.
     *
    ...
    /**
     * Get the capacity of this buffer.<p/>
     * Note that the number of bytes available plus the number of bytes free may not add up to the
     * capacity of this buffer, as the buffer may reserve some space for other purposes.
     *
     * @return the size in bytes of this buffer
     ...
     * Writing to a full buffer will block until space is available rather than throw an exception.
     */
     ...
     * Writing to a full buffer will block until space is available rather than throw an exception.<p/>
     * Note that the buffer may reserve some bytes for special purposes and capacity number of bytes may not be able to be written to the buffer.<p/>
     ...
     *
     * @param blockingWrite true writing to a full buffer should block until space is available, false if an exception should be thrown instead.
     */
    ...
    /**
     * Create a new buffer with the given capacity and blocking behavior.<p/>
     * Note that the buffer may reserve some bytes for special purposes and capacity number of bytes may not be able to be written to the buffer.<p/>
     * Note that if the buffer is of INFINITE_SIZE it will neither block or throw exceptions, but rather grow without bound.
     *
     * @param size          desired capacity of the buffer in bytes or CircularByteBuffer.INFINITE_SIZE.
    ...
    protected class CircularByteBufferInputStream extends InputStream {
        ...
        /**
         * Mark the present position in the stream. Subsequent calls to reset() will attempt to reposition the stream to this point. <p/>
         * The readAheadLimit must be less than the size of circular buffer, otherwise this method has no effect.
         *
         * @param readAheadLimit Limit on the number of bytes that may be read while still preserving the mark.
         *                       After reading this many bytes, attempting to reset the stream will fail.
         */
        @Override
         ...
         *
         * @return The byte read, as an integer in the range 0 to 255 (0x00-0xff), or -1 if the end of the stream has been reached
         * @throws java.io.IOException if the stream is closed.
         */
        @Override
        ...
        /**
         * Read bytes into an array. This method will block until some input is available,
         ...
         * at the mark. If the stream has not been marked, or more bytes than the readAheadLimit have been read, this method has no effect.
         *
         * @throws java.io.IOException if the stream is closed.
         */
        @Override
        ...
        /**
         * Skip bytes. This method will block until some bytes are available,
         * an I/O error occurs, or the end of the stream is reached.
         *
    ...
    /**
     * Class for writing to a circular byte buffer. If the buffer is full, the writes will either block
     * until there is some space available or throw an IOException based on the CircularByteBuffer's preference.
     */
    protected class CircularByteBufferOutputStream extends OutputStream {

        /**
         * Close the stream, flushing it first. This will cause the InputStream associated with this circular buffer
         * to read its last bytes once it empties the buffer. Once a stream has been closed, further write() or flush() invocations
         * will cause an IOException to be thrown. Closing a previously-closed stream, however, has no effect.
         *
         * @throws java.io.IOException never.
         */
        @Override
         ...
         * @throws java.io.IOException if buffer does not allow blocking writes and the buffer is full.
         * If the exception is thrown, no data will have been written since the buffer was set to be non-blocking.
        ...
        /**
         * Write a portion of an array of bytes. If the buffer allows blocking writes, this method will block until
         * all the data has been written rather than throw an IOException.
         *
         * @param cbuf Array of bytes
         * @param off  Offset from which to start writing bytes
         * @param len  - Number of bytes to write
         * @throws java.io.IOException if buffer does not allow blocking writes and the buffer is full.  If the exception is thrown, no data
        ...
        /**
         * Write a single byte. The byte to be written is contained in the 8 low-order bits of the
         * given integer value; the 24 high-order bits are ignored. If the buffer allows blocking writes, this method will block until
         * all the data has been written rather than throw an IOException.
         *
