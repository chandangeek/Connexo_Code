...
public class DayRecord {
    ...
    public String toString() {
        ...
        builder.append("DayRecord: dst=").append(dst).append(", holiday=").append(holiday);
        for (Object intervalData : intervalDatas) {
            IntervalData id = (IntervalData) intervalData;
...
//System.out.println("KV_DEBUG> cal="+cal.getTime()+", dst="+dst+", holiday="+holiday);
                  ...
                  if (classFactory.getAlpha().getProtocolChannelMap()!=null) {
                      if (classFactory.getAlpha().getProtocolChannelMap().isProtocolChannel(profileChannel) &&
                          ...
                          if (classFactory.getClass14LoadProfileConfiguration().getPhenomenon(profileChannel) == Class8FirmwareConfiguration.PHENOMENON_ACTIVE) {
                              ...
                              throw new IOException("DayRecord, parse(), Invalid phenomenon " + classFactory.getClass14LoadProfileConfiguration()
                                      .getPhenomenon(profileChannel) + " for profile channel " + profileChannel);
                          }
    ...
    private boolean isChecksumValidated(byte[] data) throws IOException {
       ...
       if (DEBUG>=1) {
           System.out
                   .println("KV_DEBUG> DayRecord, isChecksumValidated, datalength=0x" + Integer.toHexString(data.length) + ", checksum calculated=0x" + Integer.toHexString(checksum) + ", checksum received=0x" + Integer
                           .toHexString(rxChecksum) + ", data=" + ProtocolUtils.outputHexString(data));
       }
