...
class ObisCodeMapper {
    ...
    ObisCodeMapper(MaxSys maxSys) throws IOException {
        this.maxSys = maxSys;
        init();
    }
    ...
    Table14 getTable14( ) throws IOException {
        return maxSys.getTable14();
    }

    Table8 getTable8( ) throws IOException {
        return maxSys.getTable8();
    }
    ...
    private void init( ) throws IOException {
        ...
        int maxSelfReads = maxSys.getTable0().getTypeMaximumValues().getMaxSelfReads();
    ...
    private int getDataBlkTblSize( ) throws IOException {
        if( dataBlkTblSize == null ) {
            int dateSize = 6;
            TypeMaximumValues tmv = maxSys.getTable0().getTypeMaximumValues();
    ...
    put( o, new ValueFactory( o ){
        ...
        RegisterValue getRegisterValue( ObisCode obisCode )throws IOException{
            return new RegisterValue( obisCode, new TableAddress( maxSys, 15, offset ).readDate() );
        }
    });

    final Table14 t14 = maxSys.getTable14();

    Bus b = Bus.MTR_INPUT_BUS;
    LineSelect l = LineSelect.PLUS_WH_RECEIVED;

    Iterator si = t14.getSummations( b, l ).iterator();
    while( si.hasNext() ) {
        final TypeChannelSelectRcd rcd = (TypeChannelSelectRcd)si.next();
        o = ObisCode.fromString("1.1.1.9." + rcd.getDataBlockIndex() + "." + billing );
    ...
    b = Bus.MTR_INPUT_BUS;
    l = LineSelect.PLUS_Q1_VARH_INDUCTIVE_LAGGING;

    si = t14.getSummations( b, l ).iterator();
    while( si.hasNext() ) {
        final TypeChannelSelectRcd rcd = (TypeChannelSelectRcd)si.next();
        o = ObisCode.fromString("1.1.5.9." + rcd.getDataBlockIndex() + "." + billing );
        createValueFactory( offset, rcd, l, o );
    }
