...
public class TimeDateRTC extends AbstractRadioCommand {
    ...
    public TimeDateRTC(WaveSense waveSense) {
        super(waveSense);
    }
    ...
    protected void parse(byte[] data) throws IOException {
        if (getRadioCommandId() == RadioCommandId.ReadCurrentRTC) {
            calendar = TimeDateRTCParser.parse(data, getWaveSense().getTimeZone());
        } else {
            // check if write clock succeeded
            if (WaveflowProtocolUtils.toInt(data[0]) == SET_ERROR) {
                getWaveSense().getLogger().severe("Error setting the RTC in the wavesense device, returned [" + WaveflowProtocolUtils.toInt(data[0]) + "]");
            }
        }
    }
