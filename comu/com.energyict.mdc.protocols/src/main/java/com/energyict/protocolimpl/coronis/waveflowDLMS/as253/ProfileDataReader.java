...
public class ProfileDataReader {
    ...
    public ProfileDataReader(AS253 as253) {
        this.as253 = as253;
    }
    ...
    int readProfileInterval() throws IOException {
        AbstractDataType adt = as253.getTransparantObjectAccessFactory().readObjectAttribute(as253.getLoadProfileObisCode(), 4);
        return adt.intValue();
    }
    ...
    public ProfileData getProfileData(Date lastReading, boolean includeEvents) throws IOException {
        ...
        int profileInterval = as253.getProfileInterval();

        if (as253.isVerifyProfileInterval()) {
            profileInterval = readProfileInterval();
        }

        if (profileInterval != as253.getProfileInterval()) {
            throw new WaveFlowDLMSException("Invalid profile interval. Configured is [" + as253.getProfileInterval() + "] s, configured in meter is [" + profileInterval + "]!");
        }

        Date toDate = new Date();
        long numberOfRequestedLPEntries = 1 + ((toDate.getTime() - lastReading.getTime()) / (profileInterval * 1000));
        int max = as253.getMaxNumberOfIntervals();
        if (max != 0) {
        ...
        while (numberOfRequestedLPEntries > MAX_REQUESTED_LP_ENTRIES) {
            ...
            AbstractDataType adt = as253.getTransparantObjectAccessFactory().readObjectAttributeRange(as253.getLoadProfileObisCode(), TransparantObjectAccessFactory.ATTRIBUTE_VALUE, lastReading, toDate);
        ...
        AbstractDataType adt = as253.getTransparantObjectAccessFactory().readObjectAttributeRange(as253.getLoadProfileObisCode(), TransparantObjectAccessFactory.ATTRIBUTE_VALUE, realLastReading, toDate);
        for (AbstractDataType abstractDataType : adt.getArray().getAllDataTypes()) {
            results.addDataType(abstractDataType);      //Add the entries to the result
        }
    ...
    private List<MeterEvent> readMeterLogbook(Date lastReading) throws IOException {
        ...
        meterEvents.addAll(ErrorRegisterParser.readMeterEvents(as253));

        int applicationstatus = as253.getParameterFactory().readApplicationStatus();
        ...
        if ((applicationstatus & 0x7) != 0) {
            as253.getParameterFactory().writeApplicationStatus(0);
        }
