...
public class ContactorOpenOpticalEventLog extends AbstractEventLog {
	...
	public ContactorOpenOpticalEventLog(TimeZone timeZone) throws IOException {
		super(timeZone);
	}
	...
	public void parse(byte[] data) throws IOException {
        ...
        for( int i = 0; i < 10; i ++ ) {
    		...
    		if (timeStampInfoPairs[i].getDate() != null)
    			addMeterEvent(new MeterEvent(timeStampInfoPairs[i].getDate(), MeterEvent.OTHER,(timeStampInfoPairs[i].getInfoIndex()==1?"failed contactor open optical":"success contactor open optical")+" ("+count+")"));
        }
