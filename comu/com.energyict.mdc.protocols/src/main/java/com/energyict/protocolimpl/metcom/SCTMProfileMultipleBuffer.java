 ...
 */
abstract public class SCTMProfileMultipleBuffer implements SCTMProfileFlags {
    List frames;
        ...
        for (int bufferNr=0;bufferNr<channelMap.getNrOfBuffers();bufferNr++) {
            ...
            if (channelMap.useBuffer(bufferNr)) {
                ...
                do {
                    ...
                    for (z=0;z<status.length;z++) status[z] = frame[framePtr++];
...
//}
                    ...
                    for (int channelNr=0;channelNr<channelMap.getBuffers()[bufferNr];channelNr++) {
                        for (z=0;z<val.length;z++) val[z] = frame[framePtr++];
                        for (z=0;z<flag.length;z++) flag[z] = frame[framePtr++];
...
//}
        ...
        for (int profileNr=1;profileNr<profileDatas.length;profileNr++) {
            ...
            if ((((IntervalData)profileDatas[profileNr-1].getIntervalData(profileDatas[profileNr-1].getIntervalDatas().size()-1)).getEndTime().getTime()) != (((IntervalData)profileDatas[profileNr].getIntervalData(profileDatas[profileNr].getIntervalDatas().size()-1)).getEndTime().getTime())) {
