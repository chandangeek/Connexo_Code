...
public class ObisCodeInfo {
    ...
    public int getSelfReadIndex() {
        return Math.abs(getObisCode().getF()-254);
    }
    ...
    public boolean isContCumulativeMaximumDemand() {
        return getObisCode().getD() == ObisCodeExtensions.OBISCODE_D_CONTINUOUS_CUMULATIVE_DEMAND;
    }
    public boolean isCoinMaximumDemandDemand() {
        return (getObisCode().getD() >= ObisCodeExtensions.OBISCODE_D_COINCIDENT) && (getObisCode().getD() < (ObisCodeExtensions.OBISCODE_D_COINCIDENT+10));
    }
