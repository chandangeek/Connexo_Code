...
public abstract class AbstractDataReadingCommand {
    ...
    protected void write(String command, String data) throws ProtocolConnectionException,IOException {
        ...
        strbuff.append(data);
        strbuff.append(')');
        String str = getDataReadingCommandFactory().getSdc().getIec1107Connection().sendRawCommandFrameAndReturn(IEC1107Connection.WRITE1,strbuff.toString().getBytes());
        if (str != null)
             validateData(str);
    }
    ...
    private byte[] read(String Command,String data) throws ProtocolConnectionException,IOException {
        ...
        strbuff.append(data);
        strbuff.append(')');
        getDataReadingCommandFactory().getSdc().getIec1107Connection().sendRawCommandFrame(IEC1107Connection.READ1,strbuff.toString().getBytes());
//        getDataReadingCommandFactory().getSdc().getIec1107Connection().sendRawCommandFrame(new byte[]{'R','2'},strbuff.toString().getBytes());
