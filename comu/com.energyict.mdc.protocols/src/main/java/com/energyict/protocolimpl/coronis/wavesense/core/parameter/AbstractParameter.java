...
abstract public class AbstractParameter extends AbstractRadioCommand {
    ...
    enum ParameterId {
        ...
        HighThreshold(0x15, 2, "High threshold for alarm detection"),
        LowThreshold(0x16, 2, "Low threshold for alarm detection"),
        HighThresholdExcessTime(0x17, 1, "High threshold excess time (in multiple of the measurement period)"),
        LowThresholdExcessTime(0x18, 1, "Low threshold excess time (in multiple of the measurement period)"),
        ApplicationStatus(0x20, 1, "Application Status"),
        DetectionMeasurementPeriod(0x23, 1, "Measurement period of the threshold detection (in minutes)"),
        ...
        TimeOfMeasurement(0x83, 1, "Start time of measurement for the weekly/monthly data logging"),
        BatteryLowDectionDate(0x90, 6, "Date of low battery detection"),
        SensorFaultDectionDate(0x91, 6, "Date of sensor fault detection"),
    ...
    /**
     * The working mode of the wavesense device is implicit to the write and read command
     */
    ...
    AbstractParameter(WaveSense waveSense) {
        super(waveSense);
    }
    ...
    public void write() throws IOException {
        ...
        try {
            ...
            parseWriteResponse(getWaveSense().getWaveFlowConnect().sendData(baos.toByteArray()));
    ...
    public void read() throws IOException {
        ...
        try {
            ...
            parseReadResponse(getWaveSense().getWaveFlowConnect().sendData(baos.toByteArray()));
        }
        finally {
            if (baos != null) {
                try {
