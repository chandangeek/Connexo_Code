...
public class IDISLogBookFactory implements DeviceLogBookSupport {
    ...
    public List<CollectedLogBook> getLogBookData(List<LogBookReader> logBooks) {
        ...
        for (LogBookReader logBookReader : logBooks) {
            CollectedLogBook collectedLogBook = protocol.getCollectedDataFactory().createCollectedLogBook(logBookReader.getLogBookIdentifier());
            if (isSupported(logBookReader)) {
                ...
                try {
                    ...
                    collectedLogBook.setFailureInformation(ResultType.InCompatible, protocol.getIssueService().newWarning(logBookReader, MessageSeeds.LOGBOOK_NOT_SUPPORTED, logBookReader.getLogBookObisCode().toString(), e.getMessage()));
                }
                ...
                if (profileGeneric != null) {
                    ...
                    try {
                        if (protocol.getPhysicalAddressFromSerialNumber(logBookReader.getMeterSerialNumber()) > 0) {
                            ...
                            collectedLogBook.setMeterEvents(MeterEvent.mapMeterEventsToMeterProtocolEvents(mbusEvents, protocol.getMeteringService()));
                        } else {
                            //E-meter logbook
                ...
                collectedLogBook.setFailureInformation(ResultType.InCompatible, protocol.getIssueService().newWarning(logBookReader, MessageSeeds.LOGBOOK_NOT_SUPPORTED, logBookReader.getLogBookObisCode().toString()));
            }
    ...
    protected List<MeterProtocolEvent> parseEvents(DataContainer dataContainer, ObisCode logBookObisCode) {
        ...
        return MeterEvent.mapMeterEventsToMeterProtocolEvents(meterEvents, protocol.getMeteringService());
    }
