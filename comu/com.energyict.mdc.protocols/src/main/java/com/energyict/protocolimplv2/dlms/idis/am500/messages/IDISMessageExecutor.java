...
public class IDISMessageExecutor extends AbstractMessageExecutor {
        ...
        for (OfflineDeviceMessage pendingMessage : pendingMessages) {
            ...
            if (mBusChannelId > 0) {
                ...
                try {
                    ...
                    if (DLMSIOExceptionHandler.isUnexpectedResponse(e, getProtocol().getDlmsProperties().getRetries() + 1)) {
    ...
    protected CollectedMessage executeMessage(OfflineDeviceMessage pendingMessage, CollectedMessage collectedMessage) throws IOException {
        if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.ACTIVITY_CALENDER_SEND_WITH_DATETIME)) {
            writeActivityCalendar(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.ACTIVITY_CALENDAR_SPECIAL_DAY_CALENDAR_SEND)) {
            writeSpecialDays(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.ALARM_CONFIGURATION_RESET_ALL_ALARM_BITS)) {
        ...
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.ALARM_CONFIGURATION_RESET_ALL_ERROR_BITS)) {
            resetAllErrorBits(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.ALARM_CONFIGURATION_WRITE_ALARM_FILTER)) {
        ...
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.GENERAL_WRITE_FULL_CONFIGURATION)) {
            configurationDownload(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.CONTACTOR_CLOSE_RELAY)) {
        ...
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.CONTACTOR_OPEN)) {
            remoteDisconnect();
        ...
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.CONTACTOR_CHANGE_CONNECT_CONTROL_MODE)) {
            setControlMode(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.LOAD_BALANCING_CONFIGURE_ALL_LOAD_LIMIT_PARAMETERS)) {
        ...
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.LOAD_PROFILE_WRITE_CAPTURE_PERIOD_LP1)) {
            writeCapturePeriod(pendingMessage, 1);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.LOAD_PROFILE_WRITE_CAPTURE_PERIOD_LP2)) {
            writeCapturePeriod(pendingMessage, 2);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.MBUS_SETUP_COMMISSION)) {
            collectedMessage = commission(pendingMessage, collectedMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.PLC_CONFIGURATION_SET_TIMEOUT_NOT_ADDRESSED)) {
            setTimeoutNotAddressed(pendingMessage);
        } else if (pendingMessage.getSpecification().getId().equals(DeviceMessageId.FIRMWARE_UPGRADE_WITH_USER_FILE_AND_RESUME_OPTION_ACTIVATE_IMMEDIATE)) {
    ...
    private IDISMBusMessageExecutor getIdisMBusMessageExecutor() {
        if (idisMBusMessageExecutor == null) {
            idisMBusMessageExecutor = new IDISMBusMessageExecutor(getProtocol(), getIssueService(), getReadingTypeUtilService(), getCollectedDataFactory());
        }
    ...
    private void loadControlledConnect(OfflineDeviceMessage offlineDeviceMessage) throws IOException {
        ...
        Date emergencyProfileActivationDate = new Date(new BigDecimal(MessageConverterTools.getDeviceMessageAttribute(offlineDeviceMessage, emergencyProfileActivationDateAttributeName)
                .getDeviceMessageAttributeValue()).longValue());
    ...
    private void writeActivityCalendar(OfflineDeviceMessage offlineDeviceMessage) throws IOException {
        ...
        String codeTableDescription = MessageConverterTools.getDeviceMessageAttribute(offlineDeviceMessage, activityCalendarAttributeName).getDeviceMessageAttributeValue();
    ...
    private void writeSpecialDays(OfflineDeviceMessage offlineDeviceMessage) throws IOException {
        String codeTableDescription = MessageConverterTools.getDeviceMessageAttribute(offlineDeviceMessage, specialDaysAttributeName).getDeviceMessageAttributeValue();
