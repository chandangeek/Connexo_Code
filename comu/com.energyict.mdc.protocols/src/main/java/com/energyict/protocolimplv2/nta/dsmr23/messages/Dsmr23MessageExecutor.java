...
public class Dsmr23MessageExecutor extends AbstractMessageExecutor {
        ...
        for (OfflineDeviceMessage pendingMessage : masterMessages) {
            ...
            try {
                if (pendingMessage.getDeviceMessageId().equals(DeviceMessageId.CONTACTOR_OPEN)) {
                    ...
                    resetAlarmRegister();
                } else if (pendingMessage.getDeviceMessageId().equals(DeviceMessageId.SECURITY_KEY_RENEWAL)) {
 ...
 //       if (((KeyAccessorType)securityProperties.getTypedProperty(DlmsSecuritySupportCryptography.DATA_TRANSPORT_AUTHENTICATION_KEY_LEGACY_PROPERTY_NAME)).getName().equals(keyAccessorType)) {
   ...
   /*     if (((KeyAccessorType)securityProperties.getTypedProperty(DlmsSecuritySupportCryptography.DATA_TRANSPORT_ENCRYPTION_KEY_LEGACY_PROPERTY_NAME)).getName().equals(keyAccessorType)) {
    ...
    private TypedProperties getSecurityProperties() {
        return getProtocol().getDlmsProperties().getSecurityPropertySet().getSecurityProperties();
    }
    ...
    protected void activityCalendar(OfflineDeviceMessage pendingMessage) throws IOException {
        ...
        String activityCalendarContents = MessageConverterTools.getDeviceMessageAttribute(pendingMessage, activityCalendarAttributeName).getDeviceMessageAttributeValue();
    ...
    private void writeSpecialDays(OfflineDeviceMessage pendingMessage) throws IOException {
        String specialDayArrayBEREncodedBytes = MessageConverterTools.getDeviceMessageAttribute(pendingMessage, activityCalendarAttributeName).getDeviceMessageAttributeValue();
