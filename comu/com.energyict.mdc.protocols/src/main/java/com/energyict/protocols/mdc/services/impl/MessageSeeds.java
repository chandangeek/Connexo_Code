...
public enum MessageSeeds implements MessageSeed {
    ...
    ASYNCHRONEOUS_COMMUNICATION_IS_NOT_SUPPORTED(205, "asynchroneousCommunicationIsNotSupported", "Asynchroneous communication is not supported", Level.SEVERE),
    ...
    PROTOCOL_CONNECT(208, "protocolConnect", "The logical connect to a device failed: {0}", Level.SEVERE),
    ...
    NUMBER_OF_RETRIES_REACHED(213, "numberOfRetriesReached", "Maximum number of retries ({1}) reached: {0}", Level.SEVERE),
    ...
    SERIAL_PORT_DOES_NOT_EXIST(227, "serialPortDoesNotExist", "The ComPort with name {0} does not exist", Level.SEVERE),
    SERIAL_PORT_IS_IN_USE(228, "serialPortInUse", "The ComPort with name {0} is used by another process {1}", Level.SEVERE),
    SERIAL_PORT_LIBRARY_EXCEPTION(229, "underlyingSerialLibraryException", "Exception occurred in underlying serial library", Level.SEVERE),
    SERIAL_PORT_CONFIGURATION_MISMATCH(230, "serialPortConfigurationMismatch", "Value {1} is not expected for serial port configuration parameter {0}", Level.SEVERE),
    MODEM_COULD_NOT_HANG_UP(231, "modemCouldNotHangup", "Could not hangup/close COM port with name {0}", Level.SEVERE),
    MODEM_READ_TIMEOUT(232, "modemReadTimeout", "Modem on COM port {0} did not answer to [{2}] within configured timeout ({1} ms)", Level.SEVERE),
    MODEM_CONNECT_TIMEOUT(233, "modemConnectTimeout", "Could not connect with modem on COM port {0}, no response within timeout [{1} ms]", Level.SEVERE),
    MODEM_COULD_NOT_RESTORE_DEFAULT_PROFILE(234, "modemRestoreDefaultProfileError", "Could not restore the default modem profile settings on COM port with name {0}, request [{1}], response [{2}]", Level.SEVERE),
    MODEM_COULD_NOT_SEND_INIT_STRING(235, "modemSendInitStringError", "Failed to write init string [{1}] to modem on COM port {0}, meter response [{2}]", Level.SEVERE),
    MODEM_COULD_NOT_INITIALIZE_COMMAND_STATE(236, "modemInitCommandStateError", "Failed to initialize the command state for modem on COM port {0}, meter response [{1}]", Level.SEVERE),
    AT_MODEM_BUSY(237, "atModemBusy", "Receiver was currently busy, modem on COM port {0} returned BUSY command, last command send [{1}]", Level.SEVERE),
    AT_MODEM_ERROR(238, "atModemError", "Most likely an invalid command has been sent, modem on COM port {0} returned ERROR command, last command send [{1}]", Level.SEVERE),
    AT_MODEM_NO_ANSWER(239, "atModemNoAnswer", "Receiver was not reachable, modem on COM port {0} returned NO_ANSWER command, last command send [{1}]", Level.SEVERE),
