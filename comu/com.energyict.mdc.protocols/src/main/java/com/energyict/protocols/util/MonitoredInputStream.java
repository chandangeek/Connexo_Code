 ...
 *
 * Created on 6 oktober 2002, 12:51
 */
 ...
 * @author Karel
 */
public class MonitoredInputStream extends InputStreamDecorator {
     ...
     */
    public MonitoredInputStream(InputStream stream, InputStreamObserver observer) {
        super(stream);
        this.observer = observer;
    }

    protected InputStreamObserver getObserver() {
        return observer;
    }
    ...
    public int read() throws IOException {
        ...
        try {
            ...
            if (retValue != -1) {
                ...
                bytes[0] = (byte) retValue;
                getObserver().read(bytes);
            }
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        try {
            ...
            if (count > 0) {
                byte[] bytes = new byte[count];
                System.arraycopy(b, off, bytes, 0, bytes.length);
                getObserver().read(bytes);
            }
            return count;
        } catch (IOException ex) {
