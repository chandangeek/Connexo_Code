...
public class Installer {
    ...
    public void createTranslations(String componentName, Thesaurus thesaurus, Layer layer, MessageSeed... messageSeeds) {
        try {
            List<Translation> translations = new ArrayList<>(messageSeeds.length);
            for (MessageSeed messageSeed : messageSeeds) {
                SimpleNlsKey nlsKey = SimpleNlsKey.key(componentName, layer, messageSeed.getKey()).defaultMessage(messageSeed.getDefaultFormat());
