...
@Component(
        name = "com.energyict.mdc.device.data.audit.device.customPropertySet",
        service = {AuditTrailDecoderHandle.class},
        immediate = true)
public class AuditTrailDeviceCPSHandle implements AuditTrailDecoderHandle {
    ...
    public AuditDecoder getAuditDecoder(AuditTrailReference reference) {
        return new AuditTrailDeviceCPSDecoder(ormService, thesaurus, meteringService, serverDeviceService, customPropertySetService).init(reference);
    }
}
