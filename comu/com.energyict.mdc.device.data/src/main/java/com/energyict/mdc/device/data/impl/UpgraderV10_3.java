    ...
    private void moveProtocolDialectProperties(){
        dataModel.useConnectionRequiringTransaction(connection -> {
            try (Statement retrieveDialectPropertiesIdStatement = connection.createStatement();
                ...
                String sql = "SELECT DISTINCT NVL(DDC_COMTASKEXEC.PROTOCOLDIALECTCONFIGPROPS, DTC_PARTIALCONNECTIONTASK.DIALECTCONFIGPROPERTIES), NVL(CONNECTIONTASK, DECODE(USEDEFAULTCONNECTIONTASK, 0, NULL, 1, DDC_CONNECTIONTASK.ID )) AS CONNECTIONTASKID FROM DDC_COMTASKEXEC,DTC_PARTIALCONNECTIONTASK, DDC_CONNECTIONTASK WHERE DDC_COMTASKEXEC.DEVICE = DDC_CONNECTIONTASK.DEVICE AND DDC_CONNECTIONTASK.ISDEFAULT = 1 AND DDC_CONNECTIONTASK.PARTIALCONNECTIONTASK = DTC_PARTIALCONNECTIONTASK.ID";
