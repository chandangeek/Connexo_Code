...
public class AuditDeviceProtocolDecoder implements AuditDecoder {
    ...
    public List<AuditLogChanges> getAuditLogChanges() {
        ...
        String propertyType = Optional.of(newDeviceProtocolProperty)
                .map(dpp -> propertyValueInfoService.getConverter(((DeviceProtocolPropertyImpl) dpp.get())
                        .getPropertySpec())
                        .getPropertyType(((DeviceProtocolPropertyImpl) dpp.get()).getPropertySpec()).toString())
                .orElseGet(String::new);

        Object newPropertyValue = newDeviceProtocolProperty
                .map(dpp -> getValueFromDeviceProtocolProperty(dpp))
                .map(value -> encodeValue(propertyType, value))
                .orElseGet(String::new);
        ...
        auditLogChanges.setType(propertyType);
    ...
    private Object encodeValue(String propertyType, Object propertyValue) {
        if (propertyType.compareToIgnoreCase("DURATION") == 0) {
