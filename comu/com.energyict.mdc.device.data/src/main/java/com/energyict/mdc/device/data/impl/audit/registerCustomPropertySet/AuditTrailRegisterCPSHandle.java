...
@Component(
        name = "com.energyict.mdc.device.data.audit.register.customPropertySet",
        service = {AuditTrailDecoderHandle.class},
        immediate = true)
public class AuditTrailRegisterCPSHandle implements AuditTrailDecoderHandle {

    private final AuditDomainContextType auditDomainContextType = AuditDomainContextType.DEVICE_REGISTER_CUSTOM_ATTRIBUTES;
    ...
    public AuditDecoder getAuditDecoder(AuditTrailReference reference) {
        return new AuditTrailRegisterCPSDecoder(ormService, thesaurus, meteringService, serverDeviceService, customPropertySetService).init(reference);
    }
}
