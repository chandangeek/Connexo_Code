...
public class LogBookImplTest extends PersistenceIntegrationTest {
    ...
    private Device createSimpleDeviceWithLogBook() {
        Device device = inMemoryPersistence.getDeviceService().newDevice(deviceConfigurationWithLogBooks, "DeviceName", "MyUniqueID", Instant.now());
        device.save();
        return device;
    }
    ...
    public void createWithNoLogBooksTest() {
        Device deviceWithoutLogBooks = inMemoryPersistence.getDeviceService().newDevice(deviceConfiguration, "DeviceWithoutLogBooks", "mRID", Instant.now());
