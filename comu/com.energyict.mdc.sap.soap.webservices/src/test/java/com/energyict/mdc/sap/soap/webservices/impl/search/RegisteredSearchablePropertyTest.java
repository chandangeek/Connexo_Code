...
public class RegisteredSearchablePropertyTest {
    ...
    public void initializeMocks() {
        ...
        when(messageFormat.format(anyVararg())).thenReturn(PropertyTranslationKeys.REGISTERED.getDefaultFormat());
        when(thesaurus.getFormat(PropertyTranslationKeys.REGISTERED)).thenReturn(messageFormat);
    ...
    public void testTranslation() {
        ...
        // Asserts
        verify(this.thesaurus).getFormat(PropertyTranslationKeys.REGISTERED);
    }
    ...
    public void testAvailableOperators() {
        ...
        // Asserts
        assertThat(operators).isEqualTo(Arrays.asList(SearchablePropertyOperator.EQUAL.code(), SearchablePropertyOperator.NOT_EQUAL.code()));
    }
    ...
    public void testToSqlFragment() {
        ...
        // Asserts
        assertThat(sqlFragment).isEqualTo(fragment);
        verify(dataModel).query(eq(DeviceSAPInfoDomainExtension.class));
        verify(queryExecutor).asFragment(any(Condition.class), eq(DeviceSAPInfoDomainExtension.FieldNames.DOMAIN.javaName()));
    }

    private SearchableDeviceProperty getTestInstance() {
        return new RegisteredSearchableProperty(propertySpecService, thesaurus, ormService).init(this.parentGroup);
    }
}
