...
public class DeviceResourceTest extends DeviceDataRestApplicationJerseyTest {
    ...
    @Test
    public void testGetAttributesForDecommissionedDevice() {
        Device device = mock(Device.class, RETURNS_DEEP_STUBS);
        when(deviceService.findByUniqueMrid("COMU2102")).thenReturn(Optional.of(device));
        when(device.getmRID()).thenReturn("COMU2102");
        when(device.getVersion()).thenReturn(13L);
        when(device.getDeviceType().getId()).thenReturn(1L);
        when(device.getDeviceType().getName()).thenReturn("Actaris SL7000");
        when(device.getDeviceConfiguration().getId()).thenReturn(2L);
        when(device.getDeviceConfiguration().getName()).thenReturn("Default");
        when(device.getSerialNumber()).thenReturn("2102");
        when(device.getYearOfCertification()).thenReturn(2015);
        State state = mock(State.class);
        when(state.getId()).thenReturn(5L);
        when(state.getName()).thenReturn(DefaultState.DECOMMISSIONED.getKey());
        when(device.getState()).thenReturn(state);
        when(batchService.findBatch(device)).thenReturn(Optional.empty());
        when(device.getUsagePoint()).thenReturn(Optional.empty());
        Instant now = Instant.now();
        Instant shipmentDate = now;
        when(device.getLifecycleDates().getReceivedDate()).thenReturn(Optional.of(shipmentDate));
        Instant installationDate = now.plus(1, ChronoUnit.MILLIS);
        when(device.getLifecycleDates().getInstalledDate()).thenReturn(Optional.of(installationDate));
        Instant deactivationDate = now.plus(2, ChronoUnit.MILLIS);
        when(device.getLifecycleDates().getRemovedDate()).thenReturn(Optional.of(deactivationDate));
        Instant decommissionedDate = now.plus(3, ChronoUnit.MILLIS);
        when(device.getLifecycleDates().getRetiredDate()).thenReturn(Optional.of(decommissionedDate));

        String response = target("/devices/COMU2102/attributes").request().get(String.class);

        JsonModel jsonModel = JsonModel.model(response);
        assertDeviceAttribute(jsonModel, "mrid", "COMU2102", false, true);
        assertDeviceAttribute(jsonModel, "deviceType", "Actaris SL7000", false, true);
        assertDeviceAttribute(jsonModel, "deviceConfiguration", "Default", false, true);
        assertDeviceAttribute(jsonModel, "serialNumber", "2102", false, true);
        assertDeviceAttribute(jsonModel, "yearOfCertification", 2015, false, true);
        assertDeviceAttribute(jsonModel, "shipmentDate", shipmentDate.toEpochMilli(), true, true);
        assertDeviceAttribute(jsonModel, "installationDate", installationDate.toEpochMilli(), true, true);
        assertDeviceAttribute(jsonModel, "deactivationDate", deactivationDate.toEpochMilli(), true, true);
        assertDeviceAttribute(jsonModel, "decommissioningDate", decommissionedDate.toEpochMilli(), true, true);
    }

    private <T> void assertDeviceAttribute(JsonModel json, String name, T displayValue, boolean editable, boolean available) {
        String root = "$." + name;
        assertThat(json.<T>get(root + ".displayValue")).isEqualTo(displayValue);
        assertThat(json.<Boolean>get(root + ".editable")).isEqualTo(editable);
        assertThat(json.<Boolean>get(root + ".available")).isEqualTo(available);
    }
}