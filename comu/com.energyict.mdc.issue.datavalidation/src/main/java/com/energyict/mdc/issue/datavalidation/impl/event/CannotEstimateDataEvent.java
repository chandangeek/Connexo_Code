...
public class CannotEstimateDataEvent extends DataValidationEvent {
    ...
    public void apply(Issue issue) {
        if (issue instanceof OpenIssueDataValidation) {
            ...
            List<ReadingQualityRecord> actualReadingQualities = channel.getCimChannel(readingType).get()
                    .findReadingQualities(Collections.singleton(QualityCodeSystem.MDC), QualityCodeIndex.SUSPECT, Range.closed(startTime, endTime), true);
            actualReadingQualities.forEach(rq -> dataValidationIssue.addNotEstimatedBlock(channel, readingType, rq.getReadingTimestamp()));
        }
    }
}
