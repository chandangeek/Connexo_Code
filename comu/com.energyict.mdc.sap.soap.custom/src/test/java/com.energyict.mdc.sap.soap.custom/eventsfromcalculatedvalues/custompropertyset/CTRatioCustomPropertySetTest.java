...
public class CTRatioCustomPropertySetTest extends AbstractCustomPropertySetTest {

    private CTRatioCustomPropertySet cps;
    private PersistenceSupport<Device, CTRatioDomainExtension> persistenceSupport;

    @Override
    @Before
    public void setup() {
        super.setup();
        cps = new CTRatioCustomPropertySet(propertySpecService, thesaurus);
        persistenceSupport = cps.getPersistenceSupport();
    }

    @Test
    public void testGetId() {
        assertThat(cps.getId()).isEqualTo(CTRatioCustomPropertySet.CPS_ID);
    }

    @Test
    public void testGetName() {
        when(thesaurus.getFormat(TranslationKeys.CPS_DEVICE_CT_RATIO)).thenReturn(nlsMessageFormat);
    ...
    public void testGetPersistenceSupport() {
        assertThat(cps.getPersistenceSupport())
                .isInstanceOf(CTRatioCustomPropertySet.CustomPropertyPersistenceSupport.class);
    }
    ...
    public void testGetPropertySpecs() {
        assertThat(cps.getPropertySpecs()).hasSize(2);
    }

    @Test
    public void testComponentName() {
        assertThat(persistenceSupport.componentName()).isEqualTo(CTRatioCustomPropertySet.MODEL_NAME);
    }

    @Test
    public void testDomainFieldName() {
        assertThat(persistenceSupport.domainFieldName())
                .isEqualTo(CTRatioDomainExtension.FieldNames.DOMAIN.javaName());
    }

    @Test
    public void testPersistenceClass() {
        assertThat(persistenceSupport.persistenceClass()).isSameAs(CTRatioDomainExtension.class);
    }
    ...
    public void testAddCustomPropertyColumnsTo() {
        ...
        verify(table, times(2)).column(anyString());
    }
    ...
    public void testColumnNameFor() {
        ...
        when(propertySpec.getName()).thenReturn(CTRatioDomainExtension.FieldNames.CT_RATIO.javaName());

        assertThat(persistenceSupport.columnNameFor(propertySpec))
                .isEqualTo(CTRatioDomainExtension.FieldNames.CT_RATIO.javaName());
    }
}