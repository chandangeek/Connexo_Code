        ...
        if (firmwareCampaignOptional.isPresent()) {
            if (comTaskExecution.isFirmware()) {
                ...
                if (firmwareCampaign.isWithVerification()) {
                    Instant firmwareTimeUpload = firmwareCampaignService.findActiveFirmwareItemByDevice(comTaskExecution.getDevice()).get()
            ...
            if (comTaskEnablementOptional.isPresent()) {
                ...
                if (comTaskExecution.getConnectionTask().isPresent()) {
                    ...
                    if (comTaskExecution.getConnectionTask().get().isActive() && (!campaign.getValidationConnectionStrategy()
                        ...
                        serviceCallService.lockServiceCall(serviceCall.getId());
                        serviceCall.log(LogLevel.SEVERE, thesaurus.getFormat(MessageSeeds.CONNECTION_METHOD_DOESNT_MEET_THE_REQUIREMENT)
                ...
                serviceCallService.lockServiceCall(serviceCall.getId());
                serviceCall.log(LogLevel.SEVERE, thesaurus.getFormat(MessageSeeds.TASK_FOR_VALIDATION_IS_MISSING).format());
                serviceCall.requestTransition(DefaultState.FAILED);
            }
        }
