...
public class ProtocolUtils {
     ...
     * @throws ProtocolException Thrown in case of a parsing exception
     */
    public static void delayProtocol(int iProtocolDelayAfterFail) throws ProtocolException {
        try {
    ...
    public static String hex2String(int bKar) {
        StringBuffer strBuff = new StringBuffer();
        strBuff.append((char) convertHexLSB(bKar));
        strBuff.append((char) convertHexMSB(bKar));
        return strBuff.toString();
    }
    ...
    public static byte[] convert2ascii(byte[] data) throws ProtocolException {
        ...
        for (int iOffset = 0; iOffset < data.length / 2; iOffset++) {
            converted[iOffset] = (byte) hex2byte(data, iOffset * 2);
        }
        return converted;
    }
    ...
    public static byte[] convertBCD2ascii(byte[] data) throws ProtocolException {
        ...
        byte[] converted = new byte[data.length / 2];
        for (int iOffset = 0; iOffset < data.length / 2; iOffset++) {
            byte val = (byte) ProtocolUtils.hex2byte(data, iOffset * 2);
    ...
    public static byte hex2byte(byte[] data, int iOffset) throws ProtocolException {
        int lowNibble = 0, highNibble = 0;
        highNibble = hex2nibble(data[iOffset]) & 0xFF;
        lowNibble = hex2nibble(data[iOffset + 1]) & 0xFF;
    ...
    public static int getIntLE(ByteArrayInputStream bai) throws ProtocolException {
        return ((((int) getVal(bai)) & 0x000000FF) |
                (((int) getVal(bai) << 8) & 0x0000FF00) |
                (((int) getVal(bai) << 16) & 0x00FF0000) |
                (((int) getVal(bai) << 24) & 0xFF000000));
    }
...
//System.out.println("offset="+offset+", "+getResponseData(extendedByteBuffer));
    ...
    */
    ...
    public static int parseIntFromStr(byte[] data, int offset, int length) {
        ...
        for (int i = 0; i < length; i++) {
            stringBuffer.append((char) data[i + offset]);
        }
    ...
    public static void val2HEXascii(int val, byte[] data, int offset) {
        byte[] ascii = new byte[2];
        ascii[0] = (byte) convertHexLSB((int) val & 0xFF);
        ascii[1] = (byte) convertHexMSB((int) val & 0xFF);
        data[offset] = ascii[0];
        data[offset + 1] = ascii[1];
    }
    ...
    public static String obj2String(Object obj) throws ProtocolException {
        if (obj.getClass().getName().indexOf("Integer") != -1) {
            return ((Integer) obj).toString();
        } else if (obj.getClass().getName().indexOf("Long") != -1) {
            return ((Long) obj).toString();
        } else if (obj.getClass().getName().indexOf("BigDecimal") != -1) {
            return ((BigDecimal) obj).toString();
        } else if (obj.getClass().getName().indexOf("String") != -1) {
            return (String) obj;
        } else if (obj.getClass().getName().indexOf("Quantity") != -1) {
            return ((Quantity) obj).toString();
        } else {
     ...
     *
     * @param args arguments for unit testing
     */
    public static void main(String[] args) {
//        byte b=(byte)21;
//        byte[] test = new byte[6];
//        ProtocolUtils.val2BCDascii(b, test, 4);
//        ProtocolUtils.printResponseData(test);
//        
//        String str = "1234";
//        ProtocolUtils.printResponseData(convertAscii2Binary(str.getBytes()));
//        
...
//        ProtocolUtils.printResponseData(convertAscii2Binary(strBuff.toString().getBytes()));
//        
        try {
//            byte[] data = new byte[]{(byte)0xf7,(byte)0x00,(byte)0x57,(byte)0x00,(byte)0x00,(byte)0xfa,(byte)0xb6,(byte)0x60,(byte)0x0c,(byte)0x5e,(byte)0xe0,(byte)0x04};
//            
//            
//           // Get date time of record
//           long date = ProtocolUtils.getLongFromNibblesLE(data, 
//                                                     17,
//                                                     5).longValue();
//           
//           System.out.println(date); // (from 010100 to 311299)
//           
//           long time = ProtocolUtils.getLongFromNibblesLE(data,
//                                                     22,
//                                                     2).longValue();
//           
//           System.out.println(time); // in 1/4 hours
//           
//           Calendar calendar = ProtocolUtils.getCleanCalendar(TimeZone.getTimeZone("ECT"));
...
//           System.out.println(calendar.getTime());
            ...
            System.out.println(ProtocolUtils.getBCD2Int(new byte[]{0x11}, 0, 1));
            System.out.println(Integer.toHexString(ProtocolUtils.hex2BCD(ProtocolUtils.getBCD2Int(new byte[]{0x11}, 0, 1))));

            List<byte[]> testList = new ArrayList<byte[]>();
            testList.add(new byte[]{0, 1, 2, 3, 4});
            testList.add(new byte[]{5, 6, 7, 8, 9});
            testList.add(new byte[]{20, 21, 22, 23, 24, 25});

            byte[] result = concatListOfByteArrays(testList);
            printResponseData(result);
