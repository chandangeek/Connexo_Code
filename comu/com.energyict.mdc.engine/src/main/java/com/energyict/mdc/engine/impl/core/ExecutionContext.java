...
public final class ExecutionContext implements JournalEntryFactory {
    ...
    public void comTaskExecutionFailed(ComTaskExecution comTaskExecution) {
        connectionLogger.taskExecutionFailedDueToProblems(Thread.currentThread().getName(), comTaskExecution.getComTasks().get(0).getName(), comTaskExecution.getDevice().getName());
