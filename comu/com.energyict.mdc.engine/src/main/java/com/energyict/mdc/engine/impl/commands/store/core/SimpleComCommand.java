...
public abstract class SimpleComCommand implements ComCommand, CanProvideDescriptionTitle {
                ...
                try {
                    ...
                    if (e instanceof ConnectionCommunicationException) {
                        ...
                        if (e.getMessageSeed() == com.energyict.mdc.protocol.api.MessageSeeds.NUMBER_OF_RETRIES_REACHED_CONNECTION_STILL_INTACT) {
                            ...
                            addIssue(getServiceProvider().issueService().newProblem(deviceProtocol, getServiceProvider().thesaurus(), MessageSeeds.DEVICEPROTOCOL_PROTOCOL_ISSUE, e.getLocalizedMessage()), CompletionCode.TimeoutError);
                            ...
                            addIssue(getServiceProvider().issueService().newProblem(deviceProtocol, getServiceProvider().thesaurus(), MessageSeeds.DEVICEPROTOCOL_PROTOCOL_ISSUE, e.getLocalizedMessage()), CompletionCode.UnexpectedError);
                            getGroupedDeviceCommand().skipOtherComTaskExecutions();
                        } else {
    ...
    private void connectionErrorOccurred(DeviceProtocol deviceProtocol, Throwable e) {
        addIssue(getServiceProvider().issueService().newProblem(deviceProtocol, getServiceProvider().thesaurus(), MessageSeeds.COMMAND_FAILED_DUE_TO_CONNECTION_RELATED_ISSUE, e.getLocalizedMessage()), CompletionCode.ConnectionError);
