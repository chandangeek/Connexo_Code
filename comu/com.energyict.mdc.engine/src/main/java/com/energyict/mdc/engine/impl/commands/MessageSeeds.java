...
public enum MessageSeeds implements MessageSeed, com.energyict.mdc.upl.nls.MessageSeed {
    ...
    MAXIMUM_TIME_DIFFERENCE_EXCEEDED(5036, "maxTimeDiffExceeded", "Time difference of the device exceeds the configured maximum of {0} seconds: The difference between the clock of the device and the clock of the communication server is {1} seconds"),
    ...
    FIRMWARE_INFORMATION_NOT_SUPPORTED(5075, "firmwareInformationNotsupported", "Firmware information status isn''t supported by the protocol."),
    NOT_EXECUTED_DUE_TO_MISSING_FIRMWARE_MESSAGES(5076, "notExecutedDueToMissingFirmwareMessages", "Couldn''t find any suitable firmware message to send.");
