...
public class InboundCommunicationHandlerTest {
    ...
    private void testSuccessFulCommunication(InboundDiscoveryContextImpl context, final boolean storingSuccess) throws ExecutionException, InterruptedException {
        ...
        when(registerIdentifier.getRegisterObisCode()).thenReturn(ObisCode.fromString("1.1.1.1.1.1"));
        DefaultDeviceRegister collectedRegister = new DefaultDeviceRegister(registerIdentifier);
        ...
        when(registerType.getObisCode()).thenReturn(ObisCode.fromString("1.1.1.1.1.1"));
        when(registerGroup.getRegisterTypes()).thenReturn(Arrays.asList(registerType));
        when(registersTask.getRegisterGroups()).thenReturn(Arrays.asList(registerGroup));
