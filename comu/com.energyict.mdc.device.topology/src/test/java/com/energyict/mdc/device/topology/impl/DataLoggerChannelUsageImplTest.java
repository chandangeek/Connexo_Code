...
public class DataLoggerChannelUsageImplTest extends PersistenceIntegrationTest {
    ...
    private void setUpForSlave1Device() {
        ...
        deviceMessageIds.stream().map(DeviceMessageSpec::getId).map(DeviceMessageId::from).forEach(slave1DeviceConfiguration::createDeviceMessageEnablement);
    ...
    private void setUpForSlave2Device() {
        ...
        deviceMessageIds.stream().map(DeviceMessageSpec::getId).map(DeviceMessageId::from).forEach(slave2DeviceConfiguration::createDeviceMessageEnablement);
