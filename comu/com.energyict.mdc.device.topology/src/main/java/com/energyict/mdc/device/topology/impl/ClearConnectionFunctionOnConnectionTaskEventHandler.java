 ...
 * i.e. there is no longer a ConnectionTask using the given ConnectionFunction.
 * All {@link ComTaskExecution}s that relate to the master's slave Devices and that rely on the given
 * ConnectionFunction should be updated.
 *
...
public class ClearConnectionFunctionOnConnectionTaskEventHandler implements TopicHandler {

    public static final Logger LOGGER = Logger.getLogger(ClearConnectionFunctionOnConnectionTaskEventHandler.class.getName());

    private volatile ServerTopologyService topologyService;

    // For OSGi purposes
    public ClearConnectionFunctionOnConnectionTaskEventHandler() {
        super();
    }

    // For unit testing purposes
    ClearConnectionFunctionOnConnectionTaskEventHandler(ServerTopologyService topologyService) {
        this();
    ...
    private void handle(ConnectionTask<?, ?> connectionTask, ConnectionFunction oldConnectionFunction) {
        this.topologyService.clearConnectionTaskHavingConnectionFunctionOnComTasksInDeviceTopology(connectionTask.getDevice(), oldConnectionFunction);
    }
