...
public class UpgraderV10_7 implements Upgrader {
    ...
    private void deleteOldDestination() {
        Optional<DestinationSpec> destinationSpec = messageService.getDestinationSpec(RegisteredDevicesKpiCalculatorFactory.TASK_DESTINATION);
        destinationSpec.ifPresent(destination -> {
            destination.unSubscribe(RegisteredDevicesKpiCalculatorFactory.TASK_DESTINATION);
            destination.delete();
        });
    }
}
