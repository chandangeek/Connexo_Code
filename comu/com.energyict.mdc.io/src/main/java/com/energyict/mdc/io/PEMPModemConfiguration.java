...
public enum PEMPModemConfiguration {
     ...
     * @throws ApplicationException when no corresponding PEMPModemConfiguration could be found
     */
    public static PEMPModemConfiguration getPEMPModemConfiguration(String key) {
        ...
        throw new ApplicationException("Unknown PEMP modem configuration: '" + key + "'");
    }

    public String toString() {
        StringBuilder builder = new StringBuilder();
        builder.append(key).append(" ").append(promptResponse).append(" ").append(promptResponse).append(" ").append(connectionResponse).append(" ").append(connectionResponse);
        for (int i = 0; i < getAddresses().length; i++) {
            builder.append(" ").append(getAddresses()[i]);
        }
