...
public class RegisterTypeIsNotConfiguredException extends LocalizedException {
    ...
    public static RegisterTypeIsNotConfiguredException forChannelInLoadProfileSpec(Thesaurus thesaurus, LoadProfileSpec loadProfileSpec, MeasurementType measurementType, ChannelSpec channelSpec) {
        return new RegisterTypeIsNotConfiguredException(thesaurus, MessageSeeds.CHANNEL_SPEC_CHANNEL_TYPE_IS_NOT_IN_LOAD_PROFILE_SPEC, channelSpec.getReadingType().getAliasName(), measurementType.getReadingType().getAliasName(), loadProfileSpec.getObisCode());
    }
    ...
    public static RegisterTypeIsNotConfiguredException forChannelInDeviceType(Thesaurus thesaurus, ChannelSpec channelSpec, MeasurementType measurementType, DeviceType deviceType) {
        return new RegisterTypeIsNotConfiguredException(thesaurus, MessageSeeds.CHANNEL_SPEC_CHANNEL_TYPE_IS_NOT_ON_DEVICE_TYPE, channelSpec.getReadingType().getAliasName(), measurementType.getReadingType().getAliasName(), deviceType.getName());
    }
