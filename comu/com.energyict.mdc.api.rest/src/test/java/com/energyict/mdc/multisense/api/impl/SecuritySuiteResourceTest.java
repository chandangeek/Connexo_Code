...
public class SecuritySuiteResourceTest extends MultisensePublicApiJerseyTest {

    @Test
    public void testAllGetSecuritySuitesPaged() throws Exception {
        ...
        Response response = target("/pluggableclasses/77/securitysuites").queryParam("start", 0).queryParam("limit", 10).request().get();
        ...
        assertThat(model.<String>get("link[0].href")).isEqualTo("http://localhost:9998/pluggableclasses/77/securitysuites?start=0&limit=10");
        assertThat(model.<List>get("data")).hasSize(1);
        assertThat(model.<Integer>get("data[0].id")).isEqualTo(3);
        assertThat(model.<String>get("data[0].name")).isEqualTo("Proper name for 3");
        assertThat(model.<String>get("data[0].link.params.rel")).isEqualTo(Relation.REF_SELF.rel());
        assertThat(model.<String>get("data[0].link.href")).isEqualTo("http://localhost:9998/pluggableclasses/77/securitysuites/3");
    }
    ...
    public void testGetSingleSecuritySuiteWithFields() throws Exception {
        ...
        Response response = target("/pluggableclasses/77/securitysuites/3").queryParam("fields", "id").request().get();
        assertThat(response.getStatus()).isEqualTo(Response.Status.OK.getStatusCode());
        JsonModel model = JsonModel.model((InputStream) response.getEntity());
    ...
    @Test
    public void testSecuritySuiteFields() throws Exception {
        Response response = target("/pluggableclasses/x/securitysuites").request("application/json").method("PROPFIND", Response.class);
        JsonModel model = JsonModel.model((InputStream) response.getEntity());
