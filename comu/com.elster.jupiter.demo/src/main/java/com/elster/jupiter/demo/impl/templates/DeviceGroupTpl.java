...
public enum DeviceGroupTpl implements Template<EndDeviceGroup, DeviceGroupBuilder> {
    ...
    public DeviceGroupBuilder get(DeviceGroupBuilder builder) {
        return builder.withName(this.name).withMridPrefix(mridPrefix).withDeviceTypes(Arrays.stream(this.deviceTypes).map(DeviceTypeTpl::getName).collect(Collectors.toList()));
    }
