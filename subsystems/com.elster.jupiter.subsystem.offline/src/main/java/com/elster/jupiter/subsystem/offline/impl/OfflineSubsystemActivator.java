...
package com.elster.jupiter.subsystem.offline.impl;
...
@org.osgi.service.component.annotations.Component(name = "com.elster.jupiter.subsystem.offline", immediate = true)
public class OfflineSubsystemActivator {

    public static final String OFFLINE_ID = "Offline";
    public static final String OFFLINE_NAME = "Connexo Offline";
    public static final String APPLICATION_KEY = "OFL";

    private static final Logger LOGGER = Logger.getLogger(OfflineSubsystemActivator.class.getName());
    ...
    public void activate(BundleContext context) {
        ...
        parse(parser, findBundleResource(context, "META-INF/maven/com.elster.jupiter.subsystem/offline", "pom.xml"));
        parse(parser, findBundleResource(context, ROOT, "offline*.pom"));
        model.addDependency(buildSelfComponent(context));

        List<Component> components = model.mergeDependencies();

        subsystem = new SubsystemImpl(OFFLINE_ID, OFFLINE_NAME, context.getBundle().getVersion().toString());
    ...
    // We need to add this component manually because the offline subsystem bundle can't have itself in the list of dependencies
