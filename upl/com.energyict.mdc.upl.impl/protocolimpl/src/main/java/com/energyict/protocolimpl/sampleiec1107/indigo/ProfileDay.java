...
public class ProfileDay {
    ...
    private void parse(byte[] ba, LogicalAddressFactory laf) throws IOException {
        ...
        if (getAddressCode() != 0xFFF) {
            ...
            Calendar calendar = ProtocolUtils.getCleanCalendar(laf.getIndigoPlus().getTimeZone());
            ...
            setIntervalsPerDay((3600*24)/laf.getIndigoPlus().getProfileInterval());
            int intervalCount=0;
            ...
            if (isStatusChannel()) {
                ...
                try {
                    ...
                    for (intervalCount=0;intervalCount<getIntervalsPerDay();intervalCount++) {
                        ...
                        if (laf.getHistoricalData().isCTMeter()) {
                           dVal*=0.1;
                        }
                        else if (laf.getHistoricalData().isCTVTMeter()) {
                           dVal*=0.05;
                        }
                        else if (laf.getHistoricalData().isWholeCurrentMeter()) {
                           dVal*=1; // ??? KV TO DO 1?
                        }
                        
                        dVal*=laf.getHistoricalData().getMultiplier();
