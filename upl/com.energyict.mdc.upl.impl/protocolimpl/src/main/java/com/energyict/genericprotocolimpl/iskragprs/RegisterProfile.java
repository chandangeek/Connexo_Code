...
public class RegisterProfile{
	...
	private ArrayList profileConfiguration;
	...
	private void processDataContainer(DataContainer buffer) {
        ...
        for (int billingSetId=0;billingSetId<nrOfBillingSets;billingSetId++) {
            ...
            for ( int i = 1; i < buffer.getRoot().getStructure(billingSetId).getElements().length; i++ ){
            	billingSet.addBillingValue(getBillingValues(billingSetId, buffer, i, (ObisCode)profileConfiguration.get(i-1)));
            }
	...
	private BillingValue getBillingValues(int billingSetId, DataContainer buffer, int item, ObisCode obisCode) {
		...
		ScalerUnit sUnit = IskraMx37x.getScalerUnit(obisCode);
	...
	private BillingSet getBillingSet(int billingSetId, DataContainer buffer) {
        ...
        if (DEBUG>=1) System.out.println("DEBUG> "+billingSet);
        return billingSet;
	}
	...
	public CosemObject getValues(ObisCode obisCode) throws IOException {
        ...
        int billingPoint = billingSets.size() - 1 - (obisCode.getF() - 101);
        ...
        if (billingElement >= billingSet.getBillingValues().size())
        	return null;
        else{
	...
	private int getElement(ObisCode obisCode) throws NoSuchObjectException {
		for (int i = 0; i < profileConfiguration.size(); i++){
			ObisCode oc = (ObisCode)profileConfiguration.get(i);
			if((oc.getA()==obisCode.getA())&&(oc.getB()==obisCode.getB())&&(oc.getC()==obisCode.getC())&&(oc.getD()==obisCode.getD())&&(oc.getE()==obisCode.getE()))
				return i;
		}
	...
	public void setProfileConfiguration(ArrayList profileConfiguration) {
