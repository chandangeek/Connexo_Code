...
public class Class14LoadProfileConfiguration extends AbstractClass {
    ...
    ClassIdentification classIdentification = new ClassIdentification(14,36,true);
    
    private static final int MAX_PROFILE_CHANNELS=4;
    private static final int MAX_BLOCKS=8;
    
    int[] TBLKCF = new int[MAX_BLOCKS];
...
// kVAR received $4C    
    ...
    int[] INPUTQUANTITIES = new int[MAX_PROFILE_CHANNELS]; // load profile channels
    //00 = disable channel
    ...
    public String toString() {
        ...
        strBuff.append("Class14LoadProfileConfiguration: DTYFLG="+DTYFLG);
        strBuff.append(", LPLEN="+LPLEN);
        strBuff.append(", LPOUT="+LPOUT);
        strBuff.append(", LPMEM="+LPMEM);
        strBuff.append(", LCFLG="+LCFLG);
        for (int i=0;i<MAX_PROFILE_CHANNELS;i++) {
            strBuff.append(", INPUTQUANTITIES["+i+"]="+INPUTQUANTITIES[i]);
        }
        strBuff.append(", LPSCALE="+LPSCALE);
        
        for (int i=0;i<MAX_BLOCKS;i++) {
            strBuff.append(", TBLKCF["+i+"]=0x"+Integer.toHexString(TBLKCF[i]));
        }
    ...
    public int getDirection(int block) throws IOException {
        if (((getTBLKCF(block) >> 6) & 0x03) == 0x1)
            ...
            return 1;
        else throw new IOException("CLASS14, getDirection(), delivered AND receved are both active? (TBLKCF["+block+"]=0x"+Integer.toHexString(getTBLKCF(block)));
    }
...
//    }
    ...
    protected void parse(byte[] data) throws IOException {
        for (int block=0; block<MAX_BLOCKS; block++) {
             TBLKCF[block] = ProtocolUtils.getInt(data,block,1);
        }
        ...
        for (int i=0;i<MAX_PROFILE_CHANNELS;i++) {
           INPUTQUANTITIES[i] = ProtocolUtils.getInt(data,19+i,1);
        }
