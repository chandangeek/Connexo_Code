...
import com.energyict.protocolimpl.pact.core.instant.*;
 ...
 * @author  Koen
 * @beginchanges
KV|24082004|Add custom properties 'KeyInfoRequired' and 'StatusFlagChannel'
||Add mechanism to remove doubles and shift status flags to the next interval!
||Add 1 day extra to retrieve for the load profile! 
KV|06082004|Minor bugfix with statusflag channel
KV|26112004|If no statusflaggchannel == 0 (default) nrofchannels = surveyinfo->nrofchannels-1
KV|13012005|Changed streaming connection in PactConnection...
KV|23032005|Changed header to be compatible with protocol version tool
KV|28042005|Protect against wrong and corrupt meter configuration
KV|20062005|Add MeterType = 0 (not relevant) e.g. pulse counters
...
public class PRIPact implements MeterProtocol, ProtocolLink, RegisterProtocol {
    ...
    int extendedLogging;
    int statusFlagChannel;
    int keyInfoRequired;
    int forcedRequestExtraDays;
    int modulo;
    private int meterType;
    
    String highKeyRef=null;
    String highKey=null;
    String lowKey=null;
    
    
    PACTRegisterFactory pactRegisterFactory=null;
    PACTProfile pactProfile=null;
    PACTToolkit pactToolkit=null;
    InstantaneousFactory instantaneousFactory=null;
    ...
    public void connect() throws java.io.IOException {
        ...
        //else getPactConnection().globalEnableMeter();
    ...
    public String getFirmwareVersion() throws java.io.IOException, UnsupportedException {
    ...
    private void validateMeterIdentification() throws IOException {
        if ((strID != null) && ("".compareTo(strID) != 0)) {
            ...
            if (strID.compareTo(clemProgramName) != 0) {
                throw new IOException("PRIPact, validateMeterIdentification(), Wrong clemProgramName!, meter="+clemProgramName+", configured="+strID);   
            }
        }
...
//	}
	...
	public com.energyict.cbo.Quantity getMeterReading(String name) throws UnsupportedException, java.io.IOException {
        ...
        else return getPactRegisterFactory().getMeterReadingsInterpreter().getValue(new MeterReadingIdentifier(name));
    }
    
    public com.energyict.cbo.Quantity getMeterReading(int channelId) throws UnsupportedException, java.io.IOException {
        getPactProfile().initChannelInfo();
        return getPactRegisterFactory().getMeterReadingsInterpreter().getValueEType(getPactProfile().getLoadSurveyInterpreter().getEnergyTypeCode(channelId));
    }
    
    public int getNumberOfChannels() throws UnsupportedException, java.io.IOException {
    ...
    public String getRegister(String name) throws java.io.IOException, UnsupportedException, NoSuchRegisterException {
//        return getPactRegisterFactory().getMeterReadingsInterpreter().getValue(new MeterReadingIdentifier(name)).toString();
        return getInstantaneousFactory().getRegisterValue(name).toString();
    }

    public void setRegister(String name, String value) throws java.io.IOException, NoSuchRegisterException, UnsupportedException {
    ...
    private void validateProperties(Properties properties) throws MissingPropertyException, InvalidPropertyException {
        try {
            ...
            pactMode = new PACTMode(Integer.parseInt(properties.getProperty("PAKNET","0")), Integer.parseInt(properties.getProperty("PACTLAN","0")));
            ...
            throw new InvalidPropertyException("PRIPremier, validateProperties, NumberFormatException, "+e.getMessage());
        }
    }    
...
//    		byte[] data = getPactConnection().sendStringRequest("D");
    ...
    public void init(java.io.InputStream inputStream, java.io.OutputStream outputStream, java.util.TimeZone timeZone, java.util.logging.Logger logger) throws java.io.IOException {
    ...
    public void initializeDevice() throws java.io.IOException, UnsupportedException {
        throw new UnsupportedException();
    }
...
//            System.out.println(priPact.getProfileData(cal.getTime(),true).toString());
    ...
    final int DEFAULT_METER_TYPE=0; // no metercheck
    final int ICM200=1;
    final int CSP=2; // Calmu, Sprint, Premier
