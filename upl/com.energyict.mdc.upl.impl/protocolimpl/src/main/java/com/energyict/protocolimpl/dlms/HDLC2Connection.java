...
/**
     ...
     */
    public HDLC2Connection(InputStream inputStream,
...
//    	
	...
	private void getAddressingMode(int addressingMode) throws DLMSConnectionException {                        
        if (addressingMode == CLIENT_ADDRESSING_DEFAULT) {
            ...
            else throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, invalid server addresses");
        }
        else this.bAddressingMode=(byte)addressingMode;
        
        /*
         if ((iServerLowerMacAddress  == 0) && (iServerUpperMacAddress  <= 0x7F)) bAddressingMode = CLIENT_ADDRESSING_1BYTE;
        else if ((iServerLowerMacAddress  == 0) && (iServerUpperMacAddress  > 0x7F)) throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, invalid server addresses");
        else if ((iServerLowerMacAddress  > 0) && (iServerUpperMacAddress  != 0)) bAddressingMode = CLIENT_ADDRESSING_4BYTE;
        else throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, invalid server addresses");
         */
        
        if (iClientMacAddress > 0x7F) throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, invalid client addres");
        if (iClientMacAddress == 0) throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, client addres = 0");
        if (iServerUpperMacAddress == 0) throw new DLMSConnectionException("HDLC2Connection, getAddressingMode, unknown addressing scheme, server upper addres = 0");
    }
