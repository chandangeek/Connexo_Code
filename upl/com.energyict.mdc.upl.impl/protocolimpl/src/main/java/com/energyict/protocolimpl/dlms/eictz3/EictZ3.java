...
public class EictZ3 extends PluggableMeterProtocol implements HHUEnabler, ProtocolLink, CacheMechanism, RegisterProtocol, MessageProtocol, SerialNumberSupport {
    ...
    public final void init(final InputStream inputStream, final OutputStream outputStream, final TimeZone timeZone, final Logger logger) throws IOException {
        ...
        SecurityContext sc = new SecurityContext(this.encryptionLevel.getEncryptionValue(), this.authenticationLevel.getAuthenticationValue(), 0, "EIT12345".getBytes(), getSecurityProvider(), this.cipheringType, false);
    ...
    public final MessageResult queryMessage(final MessageEntry messageEntry) {
        if (isEpIOFirmwareUpgrade(messageEntry.getContent())) {
            ...
            if (firmwareBytes != null) {
                logger.info("Pulling out file and dispatching to the device...");
