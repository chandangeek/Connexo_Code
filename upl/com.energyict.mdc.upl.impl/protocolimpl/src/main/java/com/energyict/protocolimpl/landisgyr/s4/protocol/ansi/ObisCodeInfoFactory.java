                ...
                if (dataControlEntryIndex != 255) {
                    ...
                    if (obisCodeDescriptor != null) {
                        ...
                        for (int occurance=0;occurance<art.getNrOfOccur();occurance++)
                            obisCodeInfos.add(new ObisCodeInfo(new ObisCode(1,obisCodeDescriptor.getBField()+occurance,obisCodeDescriptor.getCField(),ObisCode.CODE_D_MAXIMUM_DEMAND,tier,fField),registerSetInfo+"max/min demand register index "+index+", "+obisCodeDescriptor.getDescription(),getFlowUnit(dataControlEntryIndex,si),index,dataControlEntryIndex, occurance));
