...
public class ProfileDataReader {
	...
	A1800 a1800;
    private int step_nr_of_entries = 125; //Max number of profile data entries we can read out at once, over RF.
...
//		int multiplier = batchObisCodeReader.intValue("Multiplier");
	...
	private List<MeterEvent> readMeterLogbook(Date lastReading, int nrOfLogEntriesInUse) throws IOException {
		...
		AbstractDataType adt = a1800.getTransparantObjectAccessFactory().readObjectAttributeEntry(AS1253.LOG_PROFILE, TransparantObjectAccessFactory.ATTRIBUTE_VALUE,nrOfLogEntriesInUse, 0);
