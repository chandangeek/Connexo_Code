...
public class ContactorArmDisconnectEventLog extends AbstractEventLog {
	...
	public ContactorArmDisconnectEventLog(TimeZone timeZone) throws IOException {
		super(timeZone);
	}
	...
	public void parse(byte[] data) throws IOException {
        ...
        for( int i = 0; i < 10; i ++ ) {
    		...
    		if (timeStampInfoPairs[i].getDate() != null)
    			addMeterEvent(new MeterEvent(timeStampInfoPairs[i].getDate(), MeterEvent.OTHER,(timeStampInfoPairs[i].getInfoIndex()==1?"failed contactor arm disconnect":"success contactor arm disconnect")+" ("+count+")"));
        }
