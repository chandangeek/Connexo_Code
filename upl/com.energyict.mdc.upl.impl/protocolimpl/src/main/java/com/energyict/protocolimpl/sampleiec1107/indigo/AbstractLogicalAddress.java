...
abstract public class AbstractLogicalAddress {
    ...
    public void retrieve() throws IOException {
        ...
        parse(data,getLogicalAddressFactory().getIndigoPlus().getTimeZone());
    }
    ...
    private void setLogicalAddress(byte[] data) throws ProtocolConnectionException,IOException {
        StringBuffer strbuff = new StringBuffer();
        strbuff.append(Integer.toHexString(getId()).toUpperCase());
        strbuff.append('(');
        strbuff.append(new String(data));
        strbuff.append(')');
        String str = getLogicalAddressFactory().getIndigoPlus().getFlagIEC1107Connection().sendRawCommandFrameAndReturn(FlagIEC1107Connection.WRITE1,strbuff.toString().getBytes());
        if (str != null)
            validateData(str);
    }
    
    private byte[] getLogicalAddress() throws ProtocolConnectionException,IOException {
        StringBuffer strbuff = new StringBuffer();
        strbuff.append(Integer.toHexString(getId()).toUpperCase());
        strbuff.append('(');
        strbuff.append(buildLength(getSize(),4).toUpperCase());
        strbuff.append(')');
        getLogicalAddressFactory().getIndigoPlus().getFlagIEC1107Connection().sendRawCommandFrame(FlagIEC1107Connection.READ1,strbuff.toString().getBytes());
    ...
    private byte[] validateData(byte[] data) throws ProtocolConnectionException {
        ...
        return getLogicalAddressFactory().getIndigoPlus().getFlagIEC1107Connection().parseDataBetweenBrackets(data);
    }
     ...
     */
    public void setLogicalAddressFactory(com.energyict.protocolimpl.sampleiec1107.indigo.LogicalAddressFactory logicalAddressFactory) {
        this.logicalAddressFactory = logicalAddressFactory;
    }    
