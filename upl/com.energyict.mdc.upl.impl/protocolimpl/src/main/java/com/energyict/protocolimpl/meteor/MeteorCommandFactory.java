...
public class MeteorCommandFactory {
	...
	MeteorCommandFactory(ComStruc s, Parsers command){
		process(s,command);
	}
	...
	private Parsers process(ComStruc s, Parsers command){
		...
		if (type){ // SEND (is always in a vector)
			this.command=Meteor.buildCommand(s.getByteArray(), null); // returns object for the receive side
			flag=true;
		}
		if (!type){ // RECEIVE (is probably in a matrix to be parsed into a vector
			// check first and last bits
			if((s.getByteArray()[0] & 0x60)==0x60){
				this.command=Meteor.buildCommand(s.getByteArray(),command);  // vector case
				flag=true;
			}
			// other cases
				...
				byteArray=bArray;
				this.command=Meteor.buildCommand(Meteor.blockMerging(byteArray),command);
				flag=true;
