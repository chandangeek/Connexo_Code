...
public class EDPStoredValues implements StoredValues {
    ...
    public static final ObisCode MONTHLY_BILLING_PROFILE = ObisCode.fromString("0.0.98.1.0.255");
    ...
    private Unit getUnit(ObisCode baseObisCode) throws IOException {
        int classID = protocol.getMeterConfig().findObject(baseObisCode).getClassID();
        if (classID == DLMSClassId.REGISTER.getClassId()) {
            return protocol.getCosemObjectFactory().getRegister(baseObisCode).getScalerUnit().getEisUnit();
        } else if (classID == DLMSClassId.EXTENDED_REGISTER.getClassId()) {
            return protocol.getCosemObjectFactory().getExtendedRegister(baseObisCode).getScalerUnit().getEisUnit();
        } else {
            throw new NoSuchRegisterException();
        }
    }
