...
public class OpusProfileParser {
    ...
    public void add(OpusResponse opusResponse) throws IOException {
        ...
        if( opusResponse == null ) return;
        if( ! opusResponse.isDefinitionMessageValid() ) return;
    ...
    /** From the meter we only get a day and a month number for a
     * profileentry.  We need to match this with a year.  To
     * do this we take the meterdate, and see if the day and month
     * of the profileentry have alread happened in this year.  If
     * not these entries are from the previous year. 
     * @throws IOException */
    ...
    class Interval {
        ...
        void addReading(byte[] reading) throws IOException {
            if ((fillIndex == 0) && (reading != null) && (reading.length > 6)) {
                int statusLength = reading.length - 6;
                status = new byte[statusLength];
                System.arraycopy(reading, 0, status, 0, statusLength);
                byteValue[fillIndex] = new byte[reading.length - statusLength];
                System.arraycopy(reading, statusLength, byteValue[fillIndex], 0,
                        (reading.length - statusLength));
            } else {
                byteValue[fillIndex] = reading;
            }
            fillIndex += 1;
        }
        ...
        boolean isPowerDown() {
            for (int i = 0; i < nrOfChannels; i++) {
                if (byteValue[i] != null
                        && byteValue[i].length == powerDownCode.length)
                    if ((powerDownCode[0] == byteValue[i][0])
                    && (powerDownCode[1] == byteValue[i][1])
                    && (powerDownCode[2] == byteValue[i][2])
                    && (powerDownCode[3] == byteValue[i][3])
                    && (powerDownCode[4] == byteValue[i][4])
                    && (powerDownCode[5] == byteValue[i][5]))
                        return true;
