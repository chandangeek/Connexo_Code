...
public class IndigoProfile {
    ...
    IndigoPlus indigoPlus;
    LogicalAddressFactory logicalAddressFactory;
    MeterExceptionInfo meterExceptionInfo;
    
    /** Creates a new instance of IndigoProfile */
    public IndigoProfile(IndigoPlus indigoPlus, MeterExceptionInfo meterExceptionInfo, LogicalAddressFactory logicalAddressFactory) {
        this.indigoPlus=indigoPlus;
    ...
    public ProfileData getProfileData(Date from, Date to) throws IOException {
        ...
        long tostd = to.getTime() + (long)indigoPlus.getTimeZone().getOffset(to.getTime());
        long fromstd = from.getTime() + (long)indigoPlus.getTimeZone().getOffset(from.getTime());
        long nrOfDaysToRetrieve = ((tostd/ONEDAY) - (fromstd/ONEDAY)) + 1;
        ProfileData profileData = doGetProfileData(nrOfDaysToRetrieve);
        profileData.sort();
        return profileData;
    }
    
    
    public ProfileData doGetProfileData(long nrOfDaysToRetrieve) throws IOException {
        ...
        for (int day=0;day<nrOfDaysToRetrieve;day++) {
            ...
            ProfileDay[] profileDays=new ProfileDay[getIndigoPlus().getNumberOfChannels()];
            
            // *********************************************************************************************************************
            // get the profileDays
            noProfileData=false;
            for (int channelIndex=0;channelIndex<getIndigoPlus().getNumberOfChannels();channelIndex++) {
                ...
                strbuff.append(')');
                getLogicalAddressFactory().getIndigoPlus().getFlagIEC1107Connection().sendRawCommandFrame(FlagIEC1107Connection.READ3,strbuff.toString().getBytes());
                byte[] ba = getLogicalAddressFactory().getIndigoPlus().getFlagIEC1107Connection().receiveRawData();
                validateData(ba);
            ...
            if (!noProfileData) { 
                int intervalsPerDay = (3600*24)/getLogicalAddressFactory().getIndigoPlus().getProfileInterval();
                ...
                Calendar calendar = ProtocolUtils.getCleanCalendar(getIndigoPlus().getTimeZone());
                ...
                for (int interval=0;interval<intervalsPerDay;interval++) {
                    ...
                    calendar.add(Calendar.SECOND,getLogicalAddressFactory().getIndigoPlus().getProfileInterval());
    ...
    private void validateData(byte[] data) throws ProtocolConnectionException {
        String str = new String(data);
        // We know about ERRDAT and ERRADD as returned error codes from the Indigo+ meter.
        // Probably there are more...
        if (str.indexOf("ERR") != -1) {
            throw new ProtocolConnectionException("IndigoProfile, validateData, "+getLogicalAddressFactory().getMeterExceptionInfo().getExceptionInfo(str));
        }
    }
...
//        }
    ...
    public IndigoPlus getIndigoPlus() {
        return indigoPlus;
    }
