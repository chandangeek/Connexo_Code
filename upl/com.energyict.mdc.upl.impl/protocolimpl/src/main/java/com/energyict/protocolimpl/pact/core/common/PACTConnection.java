...
public class PACTConnection extends Connection {
   ...
   public final int DEBUG=0;
    ...
    static public final int RQ4=0xBF; // Request Baud rate change to 4800 Baud.
    static public final int RQ9=0xBB; // Request Baud rate change to 9600 Baud.
    static public final int READLOG=0xB2; // Request the logbook
    
    static public final int BUILDTYPE=0xFE; // Request for the build type message, processor type codes and identification messages
    
    static public final int REQUEST_PASSWORD_SEED=0xC1; // Password seed request
    static public final int PASSWORD_SEED=0xC5; // Password seed
    
    static public final int PAKNET_STREAMING=0xC3; // streaming command
    
    static public final int PACTLAN_GLOBAL_ENABLE=0xEC;
    static public final int PACTLAN_GLOBAL_DISABLE=0xED;
    static public final int PACTLAN_SPECIFIC_ENABLE=0xE9;
    ...
    public PACTConnection(InputStream inputStream,OutputStream outputStream,int protocolTimeout,int maxRetries,long forceDelay,int echoCancelling) throws ConnectionException {
    ...
    public String getIntantaneousValue(String name) throws NestedIOException, ConnectionException {
        ...
        while(true) {
            if ((val = readIn()) != -1) {
                if (DEBUG >= 2) ProtocolUtils.outputHex( ((int)val));
                baos.write(val);
                if (val == '\r') 
    ...
    public void globalEnableMeter() throws NestedIOException, ConnectionException {
        ...
        while(true) {
            if ((val = readIn()) != -1) {
                if (DEBUG >= 2) ProtocolUtils.outputHex( ((int)val));
                if ((val == CFM) || (val == PACTLAN_GLOBAL_ENABLE)) return;
            } // if ((val = readIn()) != -1)
            if (((long) (System.currentTimeMillis() - timeout)) > 0) {
                return;
            }
    ...
    public void specificEnableMeter(String serialNumber) throws NestedIOException, ConnectionException {
        ...
        if (!((serialNumber != null) && ("".compareTo(serialNumber) != 0))) {
            throw new ConnectionException("PACTConnection, specificEnableMeter, serialNumber = NULL, cannot use PACTLAN specific meter enable!");
        }
        ...
        try {
            if (isConfirmed(data)) 
                ...
                throw new ConnectionException("PACTConnection, specificEnableMeter, meter returned TERMINATE instead of CONFIRM, cannot continue!");
        }
        catch(PACTConnectionException e) {
            throw new NestedIOException(e);
        }                   
    ...
    public byte[] getLoadSurveyDataStream(int nrOfBlocks,int nrOfDays) throws NestedIOException, ConnectionException {
        ...
        int dayCount=0;
        if (DEBUG >= 1) System.out.println("KV_DEBUG> getLoadSurveyDataStream(), nrOfBlocks="+nrOfBlocks+", nrOfDays="+nrOfDays);
        while(true) {
            try {
                
                if (DEBUG >= 1) System.out.println("KV_DEBUG> Try reading nrOfDays="+(nrOfDays-dayCount));
                ...
                while(true) {
                            ...
                            if (!((count == 0) && (val == CFM))) {
                                ...
                                if (count++ >= 9) {
                                   ...
                                   if (checkSum != val) { // Checksum ERROR
                                       ...
                                       if (DEBUG >= 1) System.out.println("KV_DEBUG> checksum error in block "+successblockCount+", waiting until all data has been send, timeout and retry!");
                                   }
                                   else {
                                      successblockCount++;
                                      checkSum=0;
                                      ...
                                      if ((successblockCount%nrOfBlocksPerDay)==0) {
                                         ...
                                         dayData.reset();
                                         dayCount++;
                                         if (DEBUG >= 1) System.out.println("KV_DEBUG> day "+dayCount+" read successfull!");
                                      }
                                   }

                                   if (dayCount >= nrOfDays) {
                                       if (DEBUG >= 1) System.out.println("KV_DEBUG> END! bufferstreamsize="+allData.size()+", blockCount="+successblockCount);
                                       if (DEBUG >= 1) System.out.println("KV_DEBUG> Successfull read "+dayCount+" days of "+nrOfDays);
                    ...
                    if (((long) (System.currentTimeMillis() - timeout)) > 0) {
                        ...
                        if (retries++ >= maxRetries)
                           throw new PACTConnectionException("getLoadSurveyDataStream() max retries to get "+nrOfDays+" of data",PROTOCOL_ERROR);
                        else {
                           if (DEBUG >= 1) System.out.println("KV_DEBUG> Timeout! bufferstreamsize="+allData.size()+", blockCount="+successblockCount);
                           if (DEBUG >= 1) System.out.println("KV_DEBUG> Successfull read "+dayCount+" days of "+nrOfDays);
                           break;
                        }
                    }
            ...
            try {
                ...
                while(true) {
                    if ((val = readIn()) != -1) {
                        ...
                        if (first) {
                           if ((val != 0x87) && (val != 0x8A)) { // if there was a first non (0x87 and 0x8A) byte received, wait for the second byte...
                               if (nrOfDummyBytes++ >= DUMMIES)
                                    throw new PACTConnectionException("getLoadSurveyDataStream() too many dummy bytes received while waiting for 0x87 or 0x8A...",PROTOCOL_ERROR);
                               continue;
                           }
                        }
                        ...
                        if (timeCount < 7) {
                           ...
                           if (count++ >= 9) {
                               count=0;
                               if (checkSum != val) 
                                   throw new PACTConnectionException("getMeterReadingDataStream() CRC error",CRC_ERROR);
                               checkSum=0;
                           }
                           ...
                           if (lastBlockCount > 0) {
                               if (lastBlockCount++ >= 9) {
                                   // It might be that some CLEM programs send FF as the last byte. Therefor
                                   // we wait 1000 ms to be sure that everything is received before continue and we flush the
                                   // input buffer.
                                   // See PAKNET doc!
                                   try {
                                      Thread.sleep(1000);
    ...
    private byte[] doSendControlCode(int code, boolean blockTransaction) throws NestedIOException, ConnectionException, PACTConnectionException {
...
//                	return ProtocolUtils.getSubArray2(frame, 0, length);
    ...
    private byte[] doSendStringRequest(String str) throws NestedIOException, ConnectionException, PACTConnectionException {
        ...
        while(true) {
            if ((val = readIn()) != -1) {
                ...
                if (val == 0x0D) return baos.toByteArray();
                baos.write(val);
    ...
    } // public byte[] sendStringRequest(String str) throws NestedIOException, ConnectionException, PACTConnectionException
    
    /*
     *  flushBuffer() sends 10 characters, closed by a 0x0D to the meter in order to be sure that the meter's
     *  internal buffer is empty. We should ignore all received characters and wait for 1000 ms (see doc 'introduction to metercommunication')
     *  after the last received character before returning.
     *  
     *
     */
