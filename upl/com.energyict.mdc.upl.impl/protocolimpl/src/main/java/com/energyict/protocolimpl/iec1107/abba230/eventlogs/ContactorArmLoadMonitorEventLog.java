...
public class ContactorArmLoadMonitorEventLog extends AbstractEventLog {

	int mostRecent;
	int count;
	TimeStampInfoPair[] timeStampInfoPairs = new TimeStampInfoPair[10];
	
	public ContactorArmLoadMonitorEventLog(TimeZone timeZone) throws IOException {
		super(timeZone);
	}
	...
	public void parse(byte[] data) throws IOException {
        ...
        for( int i = 0; i < 10; i ++ ) {
    		...
    		if (timeStampInfoPairs[i].getDate() != null) 
    			addMeterEvent(new MeterEvent(timeStampInfoPairs[i].getDate(), MeterEvent.OTHER,(timeStampInfoPairs[i].getInfoIndex()==1?"failed contactor arm load monitor":"success contactor arm load monitor")+" ("+count+")"));
        }
