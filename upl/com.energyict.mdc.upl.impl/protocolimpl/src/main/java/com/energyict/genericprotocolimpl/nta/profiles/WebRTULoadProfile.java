...
public class WebRTULoadProfile {
	...
	public WebRTULoadProfile(final AbstractNTAProtocol webrtu){
		this.webrtu = webrtu;
	}
	...
	public void getProfile(final ObisCode loadProfile, final boolean events) throws IOException, SQLException, BusinessException{
		...
		try {
			...
			if(channelInfos.size() != 0){
				webrtu.getLogger().log(Level.INFO, "Getting loadProfile(" + loadProfile + ") for meter with serialnumber: " + webrtu.getSerialNumberValue());
