...
public class ObisCodeMapper {
    ...
    private CosemObjectFactory cof[] = {new CosemObjectFactory(null), new CosemObjectFactory(null)};
    private RegisterProfile registerProfile[] = {new RegisterProfile(), new RegisterProfile()};
...
//  private static final ObisCode monthlyObisCode = ObisCode.fromString("1.0.98.2.0.255");
    ...
    private Object doGetRegister(ObisCode obisCode) throws IOException {
        ...
        try {
            ...
            if (obisCode.getF() == 0) {
                if (DEBUG) {
                    System.out.println("A Daily billing register");
                }
                billingPoint = -2;
            } else if (obisCode.getF() == -1) {
                if (DEBUG) {
                    System.out.println("A Monthly billing register");
                }
                billingPoint = -2;
...
//        	if ((obisCode.getF()  >=0) && (obisCode.getF() <= 99))
            ...
            if ((billingPoint == -2)) {
                ...
                switch (cofIndex) {
                    ...
                    default:
                        throw new NoSuchElementException("Only daily and monthly register reads.");
                }
...
//                                                  billingDate, fromTime, billingDate, billingDate);
            ...
            if ((obisCode.getA() == 0) && (obisCode.getB() == 0)) {
                ...
                if ((obisCode.toString().contains("0.0.128.30.21.255"))) { // Disconnector
                    registerValue = new RegisterValue(obisCode,
                            cosemObject.getQuantityValue(),
                            null, null, null,
                            new Date(), 0,
            ...
            if ((obisCode.getA() == 1) && ((obisCode.getB() == 0) || (obisCode.getB() >= 2))) {
                if (obisCode.getD() == 8) { // cumulative values, indexes
                    ...
                    if (billingPoint != -1) {
                        // get Billing timestamp
                        Data data = cof[DAILY].getData(new ObisCode(1, 0, 0, 1, 2, billingPoint));
                        return new RegisterValue(obisCode, new Quantity(BigDecimal.valueOf(register.getValue()), register.getScalerUnit()
                                .getEisUnit()), register.getCaptureTime(), data.getBillingDate());
                    } else {
