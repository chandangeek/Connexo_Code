...
public class MT83RegisterConfig extends RegisterConfig {
    ...
    private void addToMap(String obisString, int field, int startvalue, int endvalue, int deviceCodeOffset, String deviceObisMapping) {
        ...
        if (startvalue < 0x00 || startvalue > 0xFF) {
            return;
        }
        if (endvalue < 0x00 || endvalue > 0xFF) {
            return;
        }
        ...
        if (deviceObisMapping == null) {
            deviceObisMapping = obisString;
        }
        ...
        for (int i = startvalue; i <= endvalue; i++) {
            ...
            addToMap(oc.toString(), oc.toString(), doc.toString());
        }
    }
    ...
    private void addToMapInclBillingPoints(String obisString) {
        ObisCode doc;
        ObisCode oc = ObisCode.fromString(obisString);
        map.put(oc, new Register(oc.toString(), 0));
        getDeviceRegisterMapping().put(oc.toString(), oc.toString());
        ...
        for (int i = 0; i <= 14; i++) {
            ...
            map.put(oc, new Register(oc.toString(), 0));
            getDeviceRegisterMapping().put(oc.toString(), doc.toString());
        }
    }
    ...
    private void addToMap(String obisString, String deviceObisMapping) {
        this.addToMap(obisString, ObisCode.fromString(obisString).toString(), deviceObisMapping);
    }
    ...
    public ObisCode obisToDeviceCode(ObisCode obis) throws NoSuchRegisterException {
        String deviceCode = getDeviceRegisterMapping().get(obis.toString());
        if (deviceCode == null) {
    ...
    public ObisCode deviceCodeToObis(ObisCode deviceCode) throws NoSuchRegisterException {
        if (deviceCode == null) {
            throw new NoSuchRegisterException("Register " + deviceCode + " not found!");
        }
        if (!getDeviceRegisterMapping().containsValue(deviceCode.toString())) {
            return null;
        }
        String obis = null;
