...
public class FraudDetectionLog extends com.energyict.smartmeterprotocolimpl.nta.dsmr23.eventhandling.FraudDetectionLog{
    ...
    protected void buildMeterEvent(final List<MeterEvent> meterEvents, final Date eventTimeStamp, final int eventId) {
        switch(eventId){
            case EVENT_CONFIGURATION_CHANGE : {meterEvents.add(createNewFraudDetectionLogbookEvent(eventTimeStamp, MeterEvent.CONFIGURATIONCHANGE, eventId, "Indicates that configuration is activated or de-activated after the meter was installed."));}break;
