 ...
 * Time: 13:18
 */
public class MTU155FirmwareUpgradeMessage extends AbstractMTU155Message {

    private static final String MESSAGE_TAG = "FirmwareUpdate";
    ...
    public static final int LENGTH_CODE_PER_REQUEST = 1002; //Each request can contain up to 1002 bytes code.

    private SealConfig sealConfig;

    public MTU155FirmwareUpgradeMessage(MTU155MessageExecutor messageExecutor) {
    ...
    @Override
    public void executeMessage(MessageEntry messageEntry) throws BusinessException, CTRFirmwareUpgradeTimeOutException {
        super.getLogger().info("Received a Firmware Upgrade message.");
    ...
    private void loadUserFile(MessageEntry messageEntry) throws BusinessException {
		...
		if (builder.getUserFile() != null) {
            ...
            this.numberOfTotalSegments = (int) Math.ceil((double) firmwareUpgradeFile.length / (double) LENGTH_CODE_PER_REQUEST);
		} else {
    ...
    private int CheckUpgradeMode() throws BusinessException {
        try {
            ...
            if (!isPendingMessage) {
                ...
                // 5. if the current firmware version is below 0x101, the activation dates day of month should be lower than 25. (This due to a firmware bug).
                if (currentFirmwareVersion == 0x75) {
    ...
    private void doInitFirmwareUpgrade() throws BusinessException {
        try {
            super.getFactory().doInitFirmwareUpgrade(newSoftwareIdentifier, activationDate, firmwareUpgradeFile.length);
        } catch (CTRException e) {
            super.getLogger().log(Level.SEVERE, e.getMessage());
    ...
    private void doUpgrade() throws BusinessException, CTRFirmwareUpgradeTimeOutException {
        ...
        try {
            ...
            while (lastAckedSegment.getSegment() < (numberOfTotalSegments)) {
                ...
                boolean success = super.getFactory().doSendFirmwareSegment(newSoftwareIdentifier, firmwareUpgradeFile, lastAckedSegment);
                if (success) {
