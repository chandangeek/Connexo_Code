...
public class BillingDataRegisterFactoryImpl implements BillingDataRegisterFactory {
    ...
    List<BillingDataRegister>[] billingDataRegisters=null;
                ...
                switch (classFactory.getClass8FirmwareConfiguration().getBlockPhenomenon(block)) {
                    ...
                    case Class8FirmwareConfiguration.PHENOMENON_ACTIVE: {
                            ...
                            //1.1.2.128.fieldE.fieldF // coincident
                        {
                            ...
                            throw new IOException("BillingDataQuantities, build(), wrong quadrant configuration, CLASS2 EBLKCF" + block + "=" + classFactory.getClass2IdentificationAndDemandData()
                                    .getEBLKCF(block));
                        }
    ...
    private void addTotalRegisters(int set) throws IOException {
        if (!classFactory.getClass2IdentificationAndDemandData().isSingleRate()) {
            ...
            List<BillingDataRegister> extraBillingDataRegisterImpls = new ArrayList<>();
            for (BillingDataRegister bdr : billingDataRegisters[set]) {
                if (bdr.getObisCode().getE() == totalRegisterRate) {
                    ...
                    RegisterValue registerValue = new RegisterValue(obisCode, bdr.getRegisterValue().getQuantity(), bdr.getRegisterValue().getEventTime(), bdr.getRegisterValue()
                            .getFromTime(), bdr.getRegisterValue().getToTime(), bdr.getRegisterValue().getReadTime(), bdr.getRegisterValue().getRtuRegisterId(), bdr.getRegisterValue().getText());
                    extraBillingDataRegisterImpls.add(new BillingDataRegisterImpl(obisCode, bdr.getDescription(), registerValue));
                }
            }
            billingDataRegisters[set].addAll(extraBillingDataRegisterImpls);
        }
    }
    ...
    private void buildBillingDataRegisters(ClassBillingData cbd, int fieldC, int fieldE, int fieldF, int fieldEOffset, int block, int maxBlocks, int set) throws IOException {
        ...
        billingDataRegisters[set].add(new BillingDataRegisterImpl(obisCode, description, registerValue));
        ...
        if (maxBlocks == 2) {
            ...
            billingDataRegisters[set].add(new BillingDataRegisterImpl(obisCode, (block==0?"Primary":"Alternate")+"metering block coincident demand value", registerValue));
        } // if (maxBlocks == 2)
    }
