...
public class FlagIEC1107Connection extends Connection {
    ...
    private MeterType signOn(String strIdentConfig, String meterID) throws IOException,NestedIOException,FlagIEC1107ConnectionException {
        ...
        while(true) {
            try {
                ...
                if (retries++ >=iMaxRetries) throw new FlagIEC1107ConnectionException("signOn() error iMaxRetries, possibly meter not responding or wrong nodeaddress, "+e.getMessage());
                else {
                    sendBreak();
        ...
        try {
            if (iSecurityLevel != 0) {
                while(true) {
                    ...
                    if (iRetries++ >=iMaxRetries) throw new FlagIEC1107ConnectionException("Authentication error! Possibly wrong password! (error iRetries)");
                }
            }
        }
        catch (FlagIEC1107ConnectionException e) {
    ...
    public byte[] parseDataBetweenBrackets(byte[] buffer) throws FlagIEC1107ConnectionException {
        ...
        else throw new FlagIEC1107ConnectionException("FlagIEC1107Connection, parseDataBetweenBrackets, error");
    }
    ...
    private String doSendCommandFrame(byte[] command,byte[] data, boolean returnResult) throws IOException, ConnectionException, FlagIEC1107ConnectionException {
        ...
        if (command[0] == 'W') {
            while(true) {
                ...
                if (iRetries++ >=iMaxRetries) throw new FlagIEC1107ConnectionException("doSendCommandFrame() error iMaxRetries!");
            }
        }
        else if ((command[0] == 'R') || (command[0] == 'P')) {
            sendTxBuffer(); // KV 27102004
        }
        else throw new FlagIEC1107ConnectionException("doSendCommandFrame() error unknown tag!");
            ...
            if ((iNewKar = readIn()) != -1) {
                ...
                switch(state) {
                    ...
                    case STREAM_STATE_WAIT_FOR_END: {
                        ...
                        else throw new FlagIEC1107ConnectionException("receiveStreamData() invalid end flag",PROTOCOL_ERROR); // KV 27102004
                    ...
                    case STREAM_STATE_WAIT_FOR_CRC: {
                        if (count++ >= 1) {
                            ...
                            else throw new FlagIEC1107ConnectionException("receiveStreamData() bad crc",CRC_ERROR); // KV 27102004
                        }
