...
/**
 * @beginchanges
  KV  |19022004| use of RtuPlusSettings protocolretries = 5 (default)
  KV  |15032004| removed read of profileinterval from protocol
...
public class rtuplusbus implements  MeterProtocol, HalfDuplexEnabler {
        ...
        { liReceivedData = RtuPlusBusFrame.doWriteAndReadI( CMD_LOGON, RtuPlusBusFrame.doScramblePassword( lTimeOfRTU ) );
          if( liReceivedData != null )
              if( liReceivedData[0] != 0 ) { // Logged on!
                  ...
                  if (DEBUG >= 1) System.out.println(rtuPlusSettings.toString());
    ...
    public int getNumberOfChannels() throws UnsupportedException, IOException {
        ...
        if (DEBUG >= 1) System.out.println("CMD_LISTOFCHANNELS");
...
//           return 900;
    ...
    public ProfileData getProfileData(Date lastReading,boolean includeEvents) throws IOException {
        ...
        if (DEBUG >= 1) System.out.println("CMD_READ_LAST_RECORD");
        // channelValues[ iListOfChannels.length ] = Long.parseLong();
        // Get the Interavals ..
        //logger.info( "Reading last Record" );
        try{ liReceivedData = RtuPlusBusFrame.doWriteAndReadI( CMD_READ_LAST_RECORD );  }
        catch(RtuPlusBusException e) { throw new IOException( "Reading Last Record: " + e.getMessage() ); }
        ...
        liNumberOfIntervalsRetrieved = iMaximumNumberOfRecords;    // Stop after this number of records.. don't turn around for ever.
         ...
         while( (calTimeOfRecord >= calTimeOfLastReadingRecord) && (liNumberOfIntervalsRetrieved > 0) ) {
            ...
            if (DEBUG >= 1) System.out.println("calTimeOfRecord " + calendar.getTime() );
            ...
            try{
                if (DEBUG >= 1) System.out.println("CMD_READ_NEXT_RECORD");
                liNumberOfIntervalsRetrieved--;
                liReceivedData = RtuPlusBusFrame.doWriteAndReadI( CMD_READ_NEXT_RECORD );
            }
            catch(RtuPlusBusException e) { 
                if (DEBUG >= 1) System.out.println( "CMD_READ_NEXT_RECORD FAILED " + e + e.getReason() );  
                throw new NestedIOException( e );
            }
    ...
    public static String cmdToString( byte cmd ){
        switch( cmd ) {
            case 'J': return "CMD_READ_CLOCK";
            case 'd': return "CMD_FORCED_WRITE_CLOCK";
            case 'V': return "CMD_LOGON";
            case 'U': return "CMD_LOGOFF";
            case 'b': return "CMD_LISTOFCHANNELS";
            case 'i': return "CMD_READ_FIRMWAREVERSION";
            case 'K': return "CMD_READ_RTU_PARAMETERS";
            case 'S': return "CMD_READ_LAST_RECORD";
            case 'c': return "CMD_READ_NEXT_RECORD";
            case 'e': return "CMD_READ_SAME_RECORD";
            case 'L': return "CMD_READ_LOGBOOK";
