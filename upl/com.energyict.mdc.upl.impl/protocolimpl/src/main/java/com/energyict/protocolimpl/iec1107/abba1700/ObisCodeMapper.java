...
//    }
    ...
    private Object doGetRegister(ObisCode obisCode, boolean read) throws IOException {
        ...
        if (obisCode.toString().indexOf("1.1.0.1.0.255") != -1) { // billing counter
            ...
            else return new RegisterInfo("Current System Status (32 bit word)");
        }
            ...
            else return new RegisterInfo("Historical System Status (32 bit word)");
        }
        else if ((obisCode.toString().indexOf("1.1.0.4.2.255") != -1) || (obisCode.toString().indexOf("1.0.0.4.2.255") != -1)) { // CT numerator
            if (read) {
               BigDecimal bd = (BigDecimal)abba1700RegisterFactory.getRegister("CTPrimary");
            ...
            if (read) {
               BigDecimal bd = (BigDecimal)abba1700RegisterFactory.getRegister("VTPrimary");
               registerValue = new RegisterValue(obisCode,new Quantity(bd,Unit.get(255))); 
               return registerValue;
            }
            else return new RegisterInfo("VT numerator");
        }
            ...
            else return new RegisterInfo("VT denominator");
        }
