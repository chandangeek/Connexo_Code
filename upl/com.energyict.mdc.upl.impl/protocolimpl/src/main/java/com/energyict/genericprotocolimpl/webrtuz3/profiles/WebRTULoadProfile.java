...
public class WebRTULoadProfile {
	...
	public WebRTULoadProfile(){
	}
	
	public WebRTULoadProfile(final WebRTUZ3 webrtu){
		this.webrtu = webrtu;
	}
	
	public ProfileData getProfileData(final LoadProfile loadProfile) throws IOException, SQLException, BusinessException{
		this.loadProfile = loadProfile;
		return getProfile(this.loadProfile.getObisCode());
	}
	...
	public ProfileData getProfile(final ObisCode loadProfile, final boolean events) throws IOException, SQLException, BusinessException{
		final ProfileData profileData = new ProfileData( );
		ProfileGeneric genericProfile;
		
		try {
			genericProfile = getCosemObjectFactory().getProfileGeneric(loadProfile);
			final List<ChannelInfo> channelInfos = getChannelInfos(genericProfile);
			
			if(channelInfos.size() != 0){
				webrtu.getLogger().log(Level.INFO, "Getting loadProfile(" + loadProfile + ") for meter with serialnumber: " + getMeter().getSerialNumber());
