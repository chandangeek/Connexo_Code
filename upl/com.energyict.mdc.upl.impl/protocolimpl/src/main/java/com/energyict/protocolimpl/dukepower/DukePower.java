 ...
 * @exception MeterProtocolException
 */
    ...
    private ProfileData doGetProfileData(Date from, Date to,boolean includeEvents) throws IOException {
        ...
        if ((lNROfBlocksToRetrieve%256) != 0) lNROfBlocksToRetrieve+=256;
        lNROfBlocksToRetrieve/=256;

        // Check for max available blocks
        if (lNROfBlocksToRetrieve > lNROfBlocks) lNROfBlocksToRetrieve = lNROfBlocks;
        // Add 2 blocks for the event data
        if (includeEvents) lNROfBlocksToRetrieve+=2; 
        
        // beveiliging, steeds 1 block data opvragen!!
        if (lNROfBlocksToRetrieve < 2) lNROfBlocksToRetrieve = 1;
...
//System.out.println("KV_DEBUG> lNROfBlocks "+lNROfBlocks+", bNROfChannels="+bNROfChannels+", includeEvents="+includeEvents);
        ...
        if (!sendAndWaitForResponse(iProtocolTimeoutProperty,iProtocolRetriesProperty,RemoteShortResponseBuffer,MasterCommandBuffer)) {
            ...
            if (DUKEPOWERPROTOCOL_DEBUG >= 1) System.out.println(gcalendarEarliestTimeIntervalRequested.getTime());
        }
...
//System.out.println("KV_DEBUG> interval block lMBN "+lMBN);                       
    ...
    private void parseEvents(byte[] byteReceiveBuffer,ProfileData profileData,byte bYear)  throws IOException
    {
        for (int i=0;i<NR_OF_EVENTS_IN_BLOCK;i++)
        {
            if (byteReceiveBuffer[RDB_DATA+RDB_EVENT_DATA_LENGTH*i] != 0)
            {
                ...
                if (calendar.getTime().after(new Date())) calendar.add(Calendar.YEAR,-1);
    ...
    private long mapLogCodes(long lLogCode)
    {
        ...
        {
            case EV_POWER_DOWN: return(MeterEvent.POWERDOWN);
            case EV_POWER_UP: return(MeterEvent.POWERUP);
            case EV_CLOCK_SET_PREVIOUS_TIME: return(MeterEvent.SETCLOCK_BEFORE);
            case EV_CLOCK_SET_NEW_TIME: return(MeterEvent.SETCLOCK_AFTER);
            case EV_RAM_ERROR_DETECTED: return(MeterEvent.RAM_MEMORY_ERROR);
            case EV_SW_MALFUNCTION_DETECTED: return(MeterEvent.PROGRAM_FLOW_ERROR);
            case EV_METER_INPUT1_OVF: return(MeterEvent.REGISTER_OVERFLOW); 
            case EV_METER_INPUT2_OVF: return(MeterEvent.REGISTER_OVERFLOW);
            case EV_METER_INPUT3_OVF: return(MeterEvent.REGISTER_OVERFLOW);
            case EV_METER_INPUT4_OVF: return(MeterEvent.REGISTER_OVERFLOW);
            case EV_ALTER_OPTIONS: return(MeterEvent.CONFIGURATIONCHANGE);
    ...
    private void buildFrameDumpIntervalData(byte[] byteBuffer,long lNROfBlocks, boolean includeEvents)
    {
        ...
        if  (includeEvents) byteBuffer[MASTER_CMD_DATA_OFFSET+2]=0;
        else byteBuffer[MASTER_CMD_DATA_OFFSET+2]='H';
        setPassword(byteBuffer);
        clearCRC(byteBuffer);
        calcCRC(byteBuffer);
    ...
    protected void validateProperties(Properties properties) throws MissingPropertyException, InvalidPropertyException
    {
        try { 
            ...
            if (strID.length()!=7) throw new InvalidPropertyException("ID must be exact 7 characters.");
            strPassword = properties.getProperty(MeterProtocol.PASSWORD);
            if (strPassword.length()!=4) throw new InvalidPropertyException("Password must be exact 4 characters.");
...
/*************** Taken from DukePowerProtocol *****************/    
    ...
    private boolean sendAndWaitForResponse(int iTimeout,
        ...
        if (i != 0) bProtocolState |= DUKE_RETRY;
        if (i == iRetries)
        {
