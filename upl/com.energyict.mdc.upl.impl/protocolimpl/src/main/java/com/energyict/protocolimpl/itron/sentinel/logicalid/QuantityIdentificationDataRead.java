...
public class QuantityIdentificationDataRead extends AbstractDataRead {
    ...
    public String toString() {
        ...
        for (int i=0;i<getEnergyLids().length;i++) {
            try {
                strBuff.append("       energyLids["+i+"]=0x"+Long.toHexString(getEnergyLids()[i])+", "+LogicalIDFactory.findLogicalId(getEnergyLids()[i])+"\n");
            }
            catch(IOException e) {
                e.toString();
            }
        ...
        for (int i=0;i<getDemandLids().length;i++) {
            try {
                strBuff.append("       demandLids["+i+"]=0x"+Long.toHexString(getDemandLids()[i])+", "+LogicalIDFactory.findLogicalId(getDemandLids()[i])+"\n");
            }
            catch(IOException e) {
                e.toString();
            }
        ...
        for (int i=0;i<getCumDemandLids().length;i++) {
            try {
                strBuff.append("       cumDemandLids["+i+"]=0x"+Long.toHexString(getCumDemandLids()[i])+", "+LogicalIDFactory.findLogicalId(getCumDemandLids()[i])+"\n");
            }
            catch(IOException e) {
                e.toString();
            }
        }
        if (demandTOOLids != null) {
            for (int i=0;i<getDemandTOOLids().length;i++) {
                try {
                    strBuff.append("       demandTOOLids["+i+"]=0x"+Long.toHexString(getDemandTOOLids()[i])+", "+LogicalIDFactory.findLogicalId(getDemandTOOLids()[i])+"\n");
                }
                catch(IOException e) {
                    e.toString();
                }
    ...
    protected void parse(byte[] data) throws IOException {
        ...
        setDemandLids(new long[getDataReadFactory().getCapabilitiesDataRead().getNumberOfDemands()+getDataReadFactory().getCapabilitiesDataRead().getPFAvgBillingPeriodAvailable()]);
        for (int i=0;i<(getDataReadFactory().getCapabilitiesDataRead().getNumberOfDemands()+getDataReadFactory().getCapabilitiesDataRead().getPFAvgBillingPeriodAvailable());i++) {
    ...
    protected void prepareBuild() throws IOException { 
        ...
        if (getDataReadFactory().getCapabilitiesDataRead().isMeterHasAClock())
            lids = new long[]{LogicalIDFactory.findLogicalId("ALL_SEC_ENERGIES_TOTAL").getId(),
                              LogicalIDFactory.findLogicalId("ALL_SEC_DEMANDS_TOTAL").getId(),
                              LogicalIDFactory.findLogicalId("ALL_SEC_CUMS_TOTAL").getId(),
