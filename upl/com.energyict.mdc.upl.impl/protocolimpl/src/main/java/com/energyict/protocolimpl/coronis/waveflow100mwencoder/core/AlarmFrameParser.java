...
public class AlarmFrameParser {
	...
	AlarmFrameParser(byte[] data,WaveFlow100mW waveFlow100mW) throws IOException {
		...
		try {
			...
			if (waveFlow100mW.getMeterProtocolType()==MeterProtocolType.SM150E)	{
				alarmStatus = new EncoderAlarmStatus(ProtocolUtils.getSubArray(data, offset), waveFlow100mW);
			}
			else if (waveFlow100mW.getMeterProtocolType()==MeterProtocolType.ECHODIS) {
				alarmStatus = new MBusAlarmStatus(ProtocolUtils.getSubArray(data, offset), waveFlow100mW);
			}			
			
			offset += AlarmStatus.size();
			date = TimeDateRTCParser.parse(ProtocolUtils.getSubArray(data, offset), waveFlow100mW.getTimeZone()).getTime();
