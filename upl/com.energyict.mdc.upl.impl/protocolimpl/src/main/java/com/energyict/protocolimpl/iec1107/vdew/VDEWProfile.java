...
//            fos.close();
    ...
    protected List doGetLogBook(Calendar fromCalendar,Calendar toCalendar) throws IOException {
        return doGetLogBook(98, fromCalendar, toCalendar);
    }

     protected List doGetLogBook(int logBookProfileId, Calendar fromCalendar,Calendar toCalendar) throws IOException {
        byte[] responseData=null;
        List meterEvents=new ArrayList();
        try {
            responseData = readRawData(fromCalendar,toCalendar,logBookProfileId);
            if (DEBUG >= 1) {
               System.out.println("length = "+responseData.length);
               System.out.println(new String(responseData));
            }
