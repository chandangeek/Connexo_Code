                ...
                if (structure != null) {
                    if (structure.nrOfDataTypes() == 4) {
                        ...
                        if (structure.getDataType(0).getOctetString().toByteArray()[2] == obisCode.getB()) {
                            String beginDate = AXDRDate.toDescription(structure.getDataType(0).getOctetString());
                            String endDate = AXDRDate.toDescription(structure.getDataType(1).getOctetString());
                            ...
                            sb.append(beginDate).append(" - ").append(endDate).append(": ").append(sunriseTime).append(" - ").append(sunsetTime).append(NEWLINE);
                        }
                    } else {
