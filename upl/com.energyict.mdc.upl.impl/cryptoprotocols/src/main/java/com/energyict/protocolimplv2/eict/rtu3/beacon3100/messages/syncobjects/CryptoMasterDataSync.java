...
public class CryptoMasterDataSync extends MasterDataSync {
    ...
    private final CryptoBeaconMessaging cryptoBeaconMessaging;

    protected StringBuilder info = new StringBuilder();

    public CryptoMasterDataSync(CryptoBeaconMessaging beacon3100Messaging, ObjectMapperService objectMapperService, IssueFactory issueFactory, PropertySpecService propertySpecService, DeviceMasterDataExtractor deviceMasterDataExtractor, NlsService nlsService) {
        super(beacon3100Messaging, objectMapperService, issueFactory, propertySpecService, deviceMasterDataExtractor, nlsService);
        this.cryptoBeaconMessaging = beacon3100Messaging;
    }

    protected CryptoMasterDataSerializer getMasterDataSerializer() {
        return new CryptoMasterDataSerializer(getObjectMapperService(), getPropertySpecService(), getDeviceMasterDataExtractor(), getBeacon3100Properties(), getNlsService());
    }
