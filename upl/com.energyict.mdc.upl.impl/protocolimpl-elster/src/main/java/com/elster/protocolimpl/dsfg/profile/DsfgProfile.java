	...
	private void readChannelArchiveData(int index, IntervalDataMap data,
		...
		if (!roi.isOnoRangeSet()) {
			...
			do {
				...
				flRead = copyElementsToIntervallDataMap(data,
						answer, index);
				...
				request = new DataBlock(registrationInstance, 'A', 'J', 'O',
						new DataElement[] {
								...
								new DataElement(boxAddress, null, (Long) null,
										onoTop, null) });
				answer = link.getDsfgConnection().sendRequest(request);
				copyElementsToIntervallDataMap(data, answer, index);
				onoBegin = onoTop;
			}
		}
	}

	private RememberedElements copyElementsToIntervallDataMap(
			IntervalDataMap data, DataBlock answer, int index) {
