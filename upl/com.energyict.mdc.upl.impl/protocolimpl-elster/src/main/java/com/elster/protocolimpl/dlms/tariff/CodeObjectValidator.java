...
import com.elster.protocolimpl.dlms.tariff.objects.SeasonSetObject;
...
public class CodeObjectValidator {
    ...
    private static void validateDayTypes(List<CodeDayTypeObject> dayTypes) throws IllegalArgumentException {
        ...
        if (dayTypes.size() != CodeDayTypeObject.DAYTYPE_NAMES.length) {
            throw new IllegalArgumentException("Calendar needs to have [" + CodeDayTypeObject.DAYTYPE_NAMES.length + "] day types to match tariff calendar, but only had [" + dayTypes.size() + "]");
        }
    ...
    private static void validateBasicDayType(CodeDayTypeObject dayType) throws IllegalArgumentException {
        ...
        if (numberOfBands == 6) {
            ...
            throw new IllegalArgumentException("Daytype [" + dayType.getName() + "] should have only up to 6 time bands, but has [" + numberOfBands + "]");
        }
    }
