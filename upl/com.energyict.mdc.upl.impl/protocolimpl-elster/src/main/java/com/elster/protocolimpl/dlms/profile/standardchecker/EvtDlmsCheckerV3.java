 ...
 * Time: 17:29
 */
public class EvtDlmsCheckerV3 implements IArchiveLineChecker
{
    private int index = 0;
    ...
    public CheckResult check(Object[] archiveLine)
    {
        ...
        if (state != null) {
            ...
            // revision 3: error in device mode: 00(11) - unconfigured, 01 - normal, 10 - maintenance
            if (!state.isBitSet(3) || state.isBitSet(4)) {
                eisStatus |= IntervalStateBits.OTHER;
            }
            if (state.isBitSet(5)) {
