...
/**
 * Contains functionality to adjust/handle the SupplierId object
 */
public class SupplierId extends AbstractCosemObject {

    private Unsigned32 value;
    private ScalerUnit scalerUnit;

    private Unsigned32 passiveValue;
     ...
     * @param protocolLink
     */
    public SupplierId(ProtocolLink protocolLink, ObjectReference objectReference) {
     ...
     */
    public Unsigned32 getValue() throws IOException {
        if (this.value == null) {
            readValue();
        }
        return value;
     ...
     * with the {@link #writePassiveValue(com.energyict.dlms.axrdencoding.Unsigned32)} and {@link #writeActivationDate(com.energyict.dlms.axrdencoding.DateTime)}
     *
    ...
    public void writeScalerUnit(final ScalerUnit scalerUnit) throws IOException {
        write(SupplierIdAttributes.SCALER_UNIT, scalerUnit.getAbstractDataType().getBEREncodedByteArray());
     ...
     * @throws java.io.IOException if for some reason the attribute could not be read from the device
     */
    public Unsigned32 getPassiveValue() throws IOException {
    ...
    public void writePassiveScalerUnit(final ScalerUnit passiveScalerUnit) throws IOException {
        write(SupplierIdAttributes.PASSIVE_SCALER_UNIT, passiveScalerUnit.getAbstractDataType().getBEREncodedByteArray());
    ...
    public DateTime readActivationDate() throws IOException {
        this.activationDate = new DateTime(getResponseData(SupplierIdAttributes.ACTIVATION_TIME), 0);
