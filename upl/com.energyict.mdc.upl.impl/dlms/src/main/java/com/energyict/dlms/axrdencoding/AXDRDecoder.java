...
public final class AXDRDecoder {
     ...
     * @throws IOException If the given bytes could not be parsed
     */
     ...
     * @throws IOException If the given bytes could not be parsed, or the expected class type did not match with the actual type
     */
    public static <T extends AbstractDataType> T decode(byte[] data, Class<T> expectedClassType) throws IOException {
     ...
     * @throws IOException If the given bytes could not be parsed, or the expected class type did not match with the actual type
     */
    public static <T extends AbstractDataType> T decode(byte[] data, int offset, Class<T> expectedClass) throws IOException {
     ...
     * @throws IOException If the given bytes could not be parsed, or the expected class type did not match with the actual type
     */
    public static <T extends AbstractDataType> T decode(byte[] data, int offset, int level, Class<T> expectedClassType) throws IOException {
        final AbstractDataType dataType = decode(data, offset, level);
        if (!expectedClassType.isInstance(dataType)) {
            throw new IOException("Unable to decode raw axdr bytes. Expected [" + expectedClassType.getName() + "] but received [" + dataType.getClass().getName() + "]");
        }
        return (T) dataType;
    }
