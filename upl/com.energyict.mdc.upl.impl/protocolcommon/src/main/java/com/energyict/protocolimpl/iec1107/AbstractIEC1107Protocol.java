...
public abstract class AbstractIEC1107Protocol implements MeterProtocol, ProtocolLink, HHUEnabler, SerialNumber,
	 ...
	 *             If a property from the {@link #getRequiredKeys()} list was missing.
	 * @throws InvalidPropertyException
	 *             If a property has an invalid value/format
	 */
	 ...
	 * 
	 * @param requestDataReadout
	 *            true if the datadump is needed to read registers. We only use a datadump if there is no possibility in
	 *            programming mode to read registers individual. Datadump registers are always cached.
	 * 
	 * @param encryptor
	 *            interface to an encryption algorithm implemented specific for the protocol.
	 */
