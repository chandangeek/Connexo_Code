...
public class A2HHUSignOn extends IEC1107HHUSignOn {
    ...
    @Override
    public MeterType doSignOn(String strIdentConfig, String nodeId, int protocol, int mode, boolean wakeup, int baudrate) {
        int attempt = 0;
        while (true) {
            try {
                delay(1000);
                ...
                sendOut(getSNMR());
                receiveString(false);
                return new MeterTypeImpl(A2HHUHDLCConnection.A2_THEMISUNO);
            } catch (IOException e) {
    ...
    private String receiveString(boolean parityCheck) throws IOException {
        ...
        while (true) {
            ...
            if (newChar != -1) {
                ...
                byte[] hexChar = Hex.encode(new byte[]{(byte) newChar});
                String hexString = Strings.fromByteArray(hexChar);
