...
public class A2MessageExecutor extends AbstractMessageExecutor {
    ...
    private static final ObisCode UMI_DLMSWAN_CONFIGURATION = ObisCode.fromString("0.128.96.194.101.255");
    ...
    public A2MessageExecutor(AbstractDlmsProtocol protocol) {
        super(protocol);
    }

    @Override
    public CollectedMessageList executePendingMessages(List<OfflineDeviceMessage> pendingMessages) {
        CollectedMessageList result = MdcManager.getCollectedDataFactory().createCollectedMessageList(
                new DeviceIdentifierById(getProtocol().getOfflineDevice().getId()), pendingMessages);
    ...
    protected CollectedMessage executeMessage(OfflineDeviceMessage pendingMessage, CollectedMessage collectedMessage) throws IOException {
        if (pendingMessage.getSpecification().equals(FirmwareDeviceMessage.UPGRADE_FIRMWARE_WITH_USER_FILE_AND_KDL_AND_HASH_AND_ACTIVATION)) {
            ...
            // clock
        } else if (pendingMessage.getSpecification().equals(ClockDeviceMessage.SET_TIMEZONE)) {
    ...
    private void upgradeFirmware(OfflineDeviceMessage pendingMessage) throws IOException {
        String hexUserFileContent = getDeviceMessageAttributeValue(pendingMessage, firmwareUpdateUserFileAttributeName);
        String activationEpochString = getDeviceMessageAttributeValue(pendingMessage, firmwareUpdateActivationDateAttributeName);
        String hash = MessageConverterTools.getDeviceMessageAttribute(pendingMessage, firmwareUpdateHashAttributeName).getDeviceMessageAttributeValue();
        String kdl = MessageConverterTools.getDeviceMessageAttribute(pendingMessage, firmwareUpdateKDLAttributeName).getDeviceMessageAttributeValue();
        String type = MessageConverterTools.getDeviceMessageAttribute(pendingMessage, firmwareUpdateImageTypeAttributeName).getDeviceMessageAttributeValue();
        byte[] imageData = ProtocolTools.getBytesFromHexString(hexUserFileContent, "");
        byte[] typeBytes = FirmwareDeviceMessage.FirmwareImageType.typeForDescription(type).getByteArray();
