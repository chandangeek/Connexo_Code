...
public class RTU3Messaging extends AbstractMessageExecutor implements DeviceMessageSupport {
            ...
            for (Device device : masterDevice.getDownstreamDevices()) {
                ...
                for (ComTaskExecution comTaskExecution : device.getComTaskExecutions()) {
                    ...
                    if (!scheduleAlreadyExists(allMasterData.getSchedules(), comTaskExecution.getNextExecutionSpecs())) {
                        ...
                        final RTU3Schedule rtu3Schedule = new RTU3Schedule(getScheduleId(nextExecutionSpecs), getScheduleName(nextExecutionSpecs), convertToCrontabStyle(nextExecutionSpecs));
    ...
    private String convertToCrontabStyle(NextExecutionSpecs nextExecutionSpecs) {
        if (nextExecutionSpecs == null) {
            return "";
        } else if (nextExecutionSpecs.getTemporalExpression() != null) {
     ...
     */
    private int getDayOfMonth(NextExecutionSpecsShadow.Offset offset) {
     ...
     */
    private int getDayOfWeek(NextExecutionSpecsShadow.Offset offset) {
        return offset.getDays() + 1;
    }
