...
import com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet;
import com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue;
...
/**
 * My apologies for this class and as a matter of fact for entire package but I have no specs therefore copy paste from old impl...
 * TODO: rewrite this in a OOP manner
 */
public class StoredValuesImpl implements StoredValues {
     ...
     */
    public StoredValuesImpl(AbstractDlmsProtocol meterProtocol) {
    ...
    public int getBillingPointCounter() throws IOException {
        Register cosmeRegister = cof.getRegister(OBIS_NUMBER_OF_AVAILABLE_HISTORICAL_SETS);
    ...
    public Date getBillingPointTimeDate(int billingPoint) throws IOException {
        ...
        return ((com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet) billingSets.get(billingPoint)).getBillingDate();
    }
    ...
    public HistoricalValue getHistoricalValue(ObisCode obisCode) throws IOException {
        ...
        com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet billingSet = (com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet) billingSets.get(billingPoint);
        ...
        com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue billingValue = billingSet.find(registerObisCode);
    ...
    private void processDataContainer(DataContainer dc) {
        ...
        for (int billingSetId = 0; billingSetId < nrOfBillingSets; billingSetId++) {
            ...
            com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet billingSet = createNewBillingSet(billingSetId, dc);
...
//            billingSet.addBillingValues(getAllCumulativeDemands(billingSetId, CUMULATIVE_DEMAND, dc)); // cumulative demand - at the moment not used
    ...
    private com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet createNewBillingSet(int billingSetId, DataContainer dc) {
        ...
        com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingSet billingSet = new BillingSet(billingDate, billingReason, daysSinceLastReset, nrOfResets);
        return billingSet;
    }
    ...
    private List getBillingValues(int billingSetId, int typeId, DataContainer dc) {
        ...
        for (int id = 0; id < entries; id++) {
            ...
            com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue billingValue = new com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue(date, value, scalerUnit, obisCode);
            billingValues.add(billingValue);
        }
        return billingValues;
    }

    private com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue getBillingValue(int billingSetId, int typeId, DataContainer dc) {
    ...
    private com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue getAveragePF(int billingSetId, int typeId, DataContainer dc) {
        DataStructure billingSetStructure = dc.getRoot().getStructure(billingSetId);
        if (!billingSetStructure.isStructure(typeId)) {
            if (!billingSetStructure.isOctetString(typeId)) {
                ...
                com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue billingValue = new com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue(null, value, scalerUnit, obisCode);
                return billingValue;
            }
        }
        DataStructure ds = billingSetStructure.getStructure(typeId);
    ...
    private List getAllMaximumDemands(int billingSetId, int typeId, DataContainer dc) {
        ...
        for (int id = typeId; id < (typeId + MD_RANGE * 2); id += 2) {
            ...
            if (dc.getRoot().getStructure(billingSetId).getStructure(id + 1).isStructure(0)) {
                ...
                com.energyict.smartmeterprotocolimpl.actaris.sl7000.BillingValue billingValue = new BillingValue(date, value, scalerUnit, maximumDemandObisCode);
                billingValues.add(billingValue);
            }
        }
        return billingValues;
