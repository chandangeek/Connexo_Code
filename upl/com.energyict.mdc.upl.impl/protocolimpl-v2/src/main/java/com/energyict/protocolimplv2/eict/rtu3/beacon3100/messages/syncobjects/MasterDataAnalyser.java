...
public final class MasterDataAnalyser {
    ...
    private final class DeleteMirrorAction implements SyncAction<Void> {
        ...
        public String toString() {
            return "Delete mirror for MAC=" + ProtocolTools.getHexStringFromBytes(mac);
        }
    }
    ...
    private final class UpdateDeviceTypeAction implements SyncAction<Beacon3100DeviceType> {
        ...
        @Override
        public final void execute() throws IOException {
            final Structure structure = this.deviceType.toStructure(!fwVersionAbove1dot10);
     ...
     *
     * @param        The mirrors on the concentrator.
     *
     * @return The {@link ActionSet}.
     */

    /**
     * Generates the {@link ActionSet} for device types. The objects to mirror in the concentrator
     *
     * @param objectsInHES
     * @param objectsInBeaconMasterData
     * @param objectsUsedByBeaconMirrors
     * @param actionSetSupplier
     * @param <T>
     * @return The {@link ActionSet}.
     */
     ...
     */
    public final Set<Beacon3100DeviceType> getDeviceTypesInBeaconMasterdata() throws IOException {
        return getDeviceTypesInBeaconMasterdata(this.deviceTypeManager);
    }

    /**
     * Returns the device types contained in the Beacon master data, as a static generic method to be used anywhere
     *
     * @param deviceTypeManager the {@link com.energyict.dlms.cosem.DeviceTypeManager} object used to interrogate the Beacon
     * @throws IOException If an IO error occurs.
     * @return The device types contained in the beacon master data.
     */
    public static Set<Beacon3100DeviceType> getDeviceTypesInBeaconMasterdata(DeviceTypeManager deviceTypeManager) throws IOException {
        final Set<Beacon3100DeviceType> masterDataDeviceTypes = new HashSet<>();

        final Array deviceTypeArray = deviceTypeManager.readDeviceTypes();
     ...
     * @return
     */
    public final List<SyncAction<?>> analyze(final boolean fullSync) throws IOException {
        ...
        final ActionSet<Beacon3100Schedule> scheduleActionSet = this.generateActions(schedulesInHES, schedulesInBeaconMasterdata, schedulesUsedByBeaconMirrors, new ActionSetSupplier<Beacon3100Schedule>() {
            ...
            public final ActionSet<Beacon3100Schedule> createActionSet(final Set<Beacon3100Schedule> objectsToAdd, final Set<Beacon3100Schedule> objectsToUpdate, final Set<Beacon3100Schedule> objectsToRemove) {
                ...
                // Only remove stuff if we're doing a full sync, else we don't have the necessary data to know what to delete, so we'll delete a bunch of stuff that shouldn't be deleted.
                if (fullSync) {
