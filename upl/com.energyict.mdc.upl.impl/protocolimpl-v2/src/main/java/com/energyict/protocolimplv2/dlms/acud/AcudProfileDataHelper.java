...
public class AcudProfileDataHelper {
    ...
    private void readMultiplierAndScaleFactor(ObisCode obisCode) throws IOException {
        ...
        if (obisCode.equals(AcudLoadProfileDataReader.LOAD_PROFILE_PULSES)) {

            loadProfileMultiplier = AcudLoadProfileDataReader.MULTIPLIER_NON_INSTRUMENTATION;
            loadProfileScaleFactor = AcudLoadProfileDataReader.SCALE_FACTOR_NON_INSTRUMENTATION;

        } else if (obisCode.equals(AcudLoadProfileDataReader.LOAD_PROFILE_EU_CUMULATIVE) ||
                obisCode.equals(AcudLoadProfileDataReader.LOAD_PROFILE_EU_NONCUMULATIVE)) {

            loadProfileScaleFactor = AcudLoadProfileDataReader.SCALE_FACTOR_NON_INSTRUMENTATION;

        } else if (obisCode.equals(AcudLoadProfileDataReader.PROFILE_INSTRUMENTATION_SET1) ||
                obisCode.equals(AcudLoadProfileDataReader.PROFILE_INSTRUMENTATION_SET2)) {

            loadProfileMultiplier  = AcudLoadProfileDataReader.MULTIPLIER_INSTRUMENTATION;
            loadProfileScaleFactor = AcudLoadProfileDataReader.SCALE_FACTOR_INSTRUMENTATION;
