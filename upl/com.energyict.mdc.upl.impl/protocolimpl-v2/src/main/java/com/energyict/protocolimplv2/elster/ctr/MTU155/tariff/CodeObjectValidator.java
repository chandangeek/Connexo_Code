...
public class CodeObjectValidator {
    ...
    private static void validateDayTypes(List<CodeDayTypeObject> dayTypes) throws BusinessException {
        ...
        if (dayTypes.size() != CodeDayTypeObject.DAYTYPE_NAMES.length) {
            throw new BusinessException("Calendar needs to have [" + CodeDayTypeObject.DAYTYPE_NAMES.length + "] day types to match MTU155 calendar, but only had [" + dayTypes.size() + "]");
        }
    ...
    private static void validateBasicDayType(CodeDayTypeObject dayType) throws BusinessException {
        ...
        if (numberOfBands == 6) {
            ...
            throw new BusinessException("Daytype [" + dayType.getName() + "] should have 5 or 6 time bands, but has [" + numberOfBands + "]");
        }
    }
