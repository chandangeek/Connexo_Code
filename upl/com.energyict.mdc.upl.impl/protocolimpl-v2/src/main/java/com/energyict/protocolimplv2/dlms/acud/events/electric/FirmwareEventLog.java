...
public class FirmwareEventLog extends AbstractEvent {

    public FirmwareEventLog(TimeZone timeZone, DataContainer dc) {
        super(dc, timeZone);
    }
    ...
    protected void buildMeterEvent(List<MeterEvent> meterEvents, Date eventTimeStamp, int eventId, DataStructure evStructure) {
        switch (eventId) {
            case 1:
                meterEvents.add(new MeterEvent((Date) eventTimeStamp.clone(), MeterEvent.FIRMWARE_MODIFIED, eventId, "Firmware changed"));
                break;
            default:
