...
public class Dsmr23RegisterFactory implements DeviceRegisterSupport {
                ...
                if (rObisCode != null) {
                    ...
                    if (uo != null) {
                        ...
                        if (rObisCode.equals(CONNECT_CONTROL_MODE)) {
                            ...
                            dlmsAttributes.add(this.registerMap.get(register));
                        } else if (rObisCode.equals(MBUS_CLIENT_IDENTIFICATION_NUMBER)) {
                            this.registerMap.put(register, new DLMSAttribute(ObisCode.fromString("0.1.24.1.0.255"), MbusClientAttributes.IDENTIFICATION_NUMBER.getAttributeNumber(), DLMSClassId.MBUS_CLIENT.getClassId()));
                            dlmsAttributes.add(this.registerMap.get(register));
                        }else if (rObisCode.equals(MBUS_CLIENT_MANUFACTURER_ID)) {
                            this.registerMap.put(register, new DLMSAttribute(ObisCode.fromString("0.2.24.1.0.255"), MbusClientAttributes.MANUFACTURER_ID.getAttributeNumber(), DLMSClassId.MBUS_CLIENT.getClassId()));
                            dlmsAttributes.add(this.registerMap.get(register));
                        }else if (rObisCode.equals(MBUS_CLIENT_VERSION)) {
                            this.registerMap.put(register, new DLMSAttribute(ObisCode.fromString("0.3.24.1.0.255"), MbusClientAttributes.VERSION.getAttributeNumber(), DLMSClassId.MBUS_CLIENT.getClassId()));
                            dlmsAttributes.add(this.registerMap.get(register));
                        }else if (rObisCode.equals(MBUS_CLIENT_DEVICE_TYPE)) {
                            this.registerMap.put(register, new DLMSAttribute(ObisCode.fromString("0.4.24.1.0.255"), MbusClientAttributes.DEVICE_TYPE.getAttributeNumber(), DLMSClassId.MBUS_CLIENT.getClassId()));
                            dlmsAttributes.add(this.registerMap.get(register));
                        } else {
    ...
    protected RegisterValue convertCustomAbstractObjectsToRegisterValues(OfflineRegister register, AbstractDataType abstractDataType) throws UnsupportedException {
        ...
        if (rObisCode.equals(ACTIVITY_CALENDAR)) {
            ...
            return new RegisterValue(register, new Quantity(value.toBigDecimal(), Unit.getUndefined()), null, null, null, new Date(), 0, value.toString());
        } else if (rObisCode.equals(MBUS_CLIENT_MANUFACTURER_ID)) {
