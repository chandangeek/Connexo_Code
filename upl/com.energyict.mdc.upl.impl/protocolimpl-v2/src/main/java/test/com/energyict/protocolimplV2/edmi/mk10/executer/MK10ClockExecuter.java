...
public class MK10ClockExecuter {
    ...
    private boolean isCrossBoundary(Date systemDate, Date meterDate, MeterProtocol meterProtocol, int iProfileInterval) {
    ...
    private long getDiff(Date systemDate, Date date) {
	...
	private MK10Generic getMk10Protocol() {
    ...
    public long verifyAndSetMeterTime(TimeZone timeZone) throws IOException, BusinessException {
        ...
        Date systemDate = Calendar.getInstance().getTime();	// get the system time
        Date meterDate = getExecuter().getMk10Protocol().getTime();			// get the meter time (NOT roundtrip corrected !!!)
    ...
    public boolean verifyMeterMaxTimeDiff(boolean collectOutsiteMaxTimeDiff) throws IOException, BusinessException {
        ...
        Date meterDate = getMk10Protocol().getTime();
