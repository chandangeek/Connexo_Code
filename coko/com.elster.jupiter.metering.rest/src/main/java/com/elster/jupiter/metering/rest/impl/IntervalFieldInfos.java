...
public class IntervalFieldInfos {
    ...
    public IntervalFieldInfos from(Set<TimeAttribute> timeAttributes, Set<MacroPeriod> macroPeriods, Thesaurus thesaurus) {
        add(TimeAttribute.NOTAPPLICABLE.getId(), thesaurus.getString(MessageSeeds.Keys.TIME_ATTRIBUTE_KEY_PREFIX + TimeAttribute.NOTAPPLICABLE.getId(),
        ...
        timeAttributes.stream()
                ...
                .forEach(ta -> this.add(ta.getId(), thesaurus.getString(MessageSeeds.Keys.TIME_ATTRIBUTE_KEY_PREFIX + ta.getId(), ta.getDescription()), null));
        macroPeriods.stream()
                .sorted((mp1, mp2) -> mp1.getDescription().compareTo(mp2.getDescription()))
                .filter(mp -> mp.getId() != 0)
                .forEach(mp -> this.add(null, thesaurus.getString(MessageSeeds.Keys.MACRO_PERIOD_KEY_PREFIX + mp.getId(), mp.getDescription()), mp.getId()));
        return this;
    }
}