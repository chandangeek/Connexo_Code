...
public class ServiceCategoryResource {
    ...
    public PagedInfoList getServiceCategories(@PathParam("kind") String kind, @BeanParam JsonQueryParameters queryParameters, @Context SecurityContext securityContext) {
        ...
        List<CustomPropertySetInfo> infos = category.getCustomPropertySets().stream().map(customPropertySetInfoFactory::getGeneralAndPropertiesInfo).sorted((a, b) -> a.name.compareToIgnoreCase(b.name)).collect(Collectors.toList());
