...
Ext.define('Itk.view.ActionMenu', {
    ...
    addDynamicActions: function () {
        ...
        me.store.each(function (record) {
            ...
            switch (record.get('name')) {
                case 'Assign issue':
                    privileges = Itk.privileges.Issue.canDoAction() && Itk.privileges.Issue.assign;
                    section = this.SECTION_ACTION;
                    break;
                case 'Close issue':
                    privileges = Itk.privileges.Issue.canDoAction() && Itk.privileges.Issue.close;
                    section = this.SECTION_REMOVE;
                    break;
                case 'Set priority for issue(s)':
                    privileges = Itk.privileges.Issue.canDoAction() && Itk.privileges.Issue.setPriority;
                    section = this.SECTION_REMOVE;
                    break;
                case 'Snooze':
                    privileges = Itk.privileges.Issue.canDoAction();
                    section = this.SECTION_ACTION;
                    break;
            }
    ...
    onCheck: function (getConfirmationWindow) {
        ...
        me.doOperation(confWindow, Uni.I18n.translate('snooze.acknowledgment.snoozed', 'ITK', 'Issue snoozed'), 'snooze');
    },
    ...
    snoozeCheck: function (item) {
        if (item.action === "snooze") {
            ...
            confirmationWindow = Ext.create('Uni.view.window.Confirmation', {
                ...
                confirmText: Uni.I18n.translate('issue.snooze', 'ITK', 'Snooze'),
                closeAction: 'destroy',
                green: true,
            ...
            confirmationWindow.show({
                title: Uni.I18n.translate('issue.snoozeNow', 'ITK', "Snooze '{0}'?",
    ...
    addPredefinedActions: function () {
        var me = this,
            itemId = me.record.getId(),
            detail = Ext.ComponentQuery.query('issue-detail-top')[0],
        ...
        if (Itk.privileges.Issue.viewAdminProcesses) {
            me.add({

                text: Uni.I18n.translate('issues.actionMenu.startProcess', 'ITK', 'Start process'),
                action: 'startProcess',
                section: this.SECTION_ACTION,
                href: me.router.getRoute(me.router.currentRoute.replace('/view', '') + '/view/startProcess').buildUrl({issueId: itemId}, {details: (detail) ? true : false}),
                details: false,
    ...
    getPredefinedItems: function () {
        ...
        return [
            {
                text: Uni.I18n.translate('issues.actionMenu.assignToMe', 'ITK', 'Assign to me'),
                privileges: Itk.privileges.Issue.assign,
                action: 'assignIssueToMe',
                itemId: 'assign-issue-to-me',
                section: me.SECTION_ACTION,
                hidden: true
            },
            {
                text: Uni.I18n.translate('issues.actionMenu.unassign', 'ITK', 'Unassign'),
                privileges: Itk.privileges.Issue.assign,
                action: 'unassign',
                itemId: 'unassign-issue',
                section: me.SECTION_ACTION,
                hidden: true
            },
            {
                text: Uni.I18n.translate('issues.actionMenu.snooze', 'ITK', 'Snooze'),
            ...
            {
                text: Uni.I18n.translate('issues.actionMenu.addComment', 'ITK', 'Add comment'),
                privileges: Itk.privileges.Issue.comment,
                section: me.SECTION_ACTION,
                action: 'addComment'
            },
            {
                text: Uni.I18n.translate('issues.actionMenu.setPriority', 'ITK', 'Set priority'),
