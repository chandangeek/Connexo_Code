...
public enum TableSpecs {
    ...
    private static class TableBuilder {
        ...
        static void buildIssueTable(Table<?> table, Column idColumn, String pkKey, String... fkKeys) {
            ...
            table.column(ISSUE_COLUMN_URGENCY).map("urgency").number().conversion(NUMBER2LONG).notNull().add();
            table.column(ISSUE_COLUMN_IMPACT).map("impact").number().conversion(NUMBER2LONG).notNull().add();
