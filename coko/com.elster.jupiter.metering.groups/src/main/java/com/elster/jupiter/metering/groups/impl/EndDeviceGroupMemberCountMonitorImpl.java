...
/**
 * Provides an implementation for the {@link EndDeviceGroupMemberCountMonitor} interface.
 *
 * @author Rudi Vankeirsbilck (rudi)
 * @since 2016-03-08 (16:27)
 */
@Component(name = "com.elster.jupiter.metering.groups.monitor.jmx",
        property = {"jmx.objectname=com.elster.jupiter:type=EndDeviceGroupMemberCountMonitor"})
public class EndDeviceGroupMemberCountMonitorImpl implements EndDeviceGroupMemberCountMonitor, EndDeviceGroupMemberCountMonitorImplMBean {
    private ExecutionStatisticsImpl memberCount;

    // For OSGi purposes
    public EndDeviceGroupMemberCountMonitorImpl() {
        super();
    }

    @Inject
    public EndDeviceGroupMemberCountMonitorImpl(NlsService nlsService) {
        this();
    ...
    public void setNlsService(NlsService nlsService) {
        Thesaurus thesaurus = nlsService.getThesaurus(MeteringGroupsService.COMPONENTNAME, Layer.DOMAIN);
        this.memberCount = new ExecutionStatisticsImpl(thesaurus);
    }
    ...
    @Override
    public void countExecuted(long nanos) {
        synchronized (this.memberCount) {
            this.memberCount.registerExecution(nanos);
        }
    }
        ...
        private CompositeType getCompositeType () {
            try {
                return new CompositeType(
                        EndDeviceGroupMemberCountMonitorImpl.class.getSimpleName(),
