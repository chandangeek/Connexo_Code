...
class MeterReadingsFactory {
    ...
    MeterReadings build(ReadingStorer readingStorer) {
        ...
        referencedReadingTypes.stream()
                .map(MeterReadingsFactory::createReadingType)
                .forEach(readingTypeList::add);
        // filled in scope of wrapInMeterReading
        referencedReadingQualityTypes.stream()
                .map(MeterReadingsFactory::createReadingQualityType)
    ...
    private Reading createReading(ReadingType readingType, ReadingWithQualities record) {
        ...
        record.getReading().ifPresent(reading -> {
            ...
            reading.getTimePeriod()
                    .map(MeterReadingsFactory::createDateTimeInterval)
