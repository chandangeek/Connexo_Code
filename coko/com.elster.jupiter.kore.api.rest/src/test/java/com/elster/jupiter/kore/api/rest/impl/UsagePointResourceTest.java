...
public class UsagePointResourceTest extends PlatformPublicApiJerseyTest {
    ...
    public void testAddMetrologyIfOneExisted() throws Exception {
        ...
        UsagePoint usagePoint = mockUsagePoint(11L, "usage point", 2L, ServiceKind.ELECTRICITY);
        EffectiveMetrologyConfigurationOnUsagePoint oldEffectiveMetrologyConfiguration = mock(EffectiveMetrologyConfigurationOnUsagePoint.class);
        EffectiveMetrologyConfigurationOnUsagePoint newEffectiveMetrologyConfiguration = mock(EffectiveMetrologyConfigurationOnUsagePoint.class);
        when(usagePoint.getCurrentEffectiveMetrologyConfiguration()).thenReturn(Optional.of(oldEffectiveMetrologyConfiguration));
        when(oldEffectiveMetrologyConfiguration.getMetrologyConfiguration()).thenReturn(oldMetrologyConfiguration);
        when(newEffectiveMetrologyConfiguration.getMetrologyConfiguration()).thenReturn(newMetrologyConfiguration);
        when(usagePoint.getEffectiveMetrologyConfiguration(Instant.ofEpochMilli(235L))).thenReturn(Optional.of(newEffectiveMetrologyConfiguration));
        when(newEffectiveMetrologyConfiguration.getUsagePoint()).thenReturn(usagePoint);
        when(oldEffectiveMetrologyConfiguration.getUsagePoint()).thenReturn(usagePoint);
        when(newEffectiveMetrologyConfiguration.getRange()).thenReturn(Range.atLeast(clock.instant()));
        when(oldEffectiveMetrologyConfiguration.getRange()).thenReturn(Range.atLeast(clock.instant()));
        ElectricityDetail electricityDetail = mock(ElectricityDetail.class);
        ElectricityDetailBuilder electricityDetailBuilder = FakeBuilder.initBuilderStub(electricityDetail, ElectricityDetailBuilder.class);
        when(usagePoint.newElectricityDetailBuilder(any())).thenReturn(electricityDetailBuilder);
