...
public class ServiceCallIssueEventHandlerTest extends BaseTest {

    @Test
    public void testUnmappedEvent() {
        Map<String, Object> messageMap = new HashMap<>();
        messageMap.put("serviceCallId", 1);
        messageMap.put("newState", 6);
        Message message = getMockMessage(getJsonService().serialize(messageMap));
        MessageHandler handler = getServiceCallIssueEventHandler(getMockIssueCreationService());
    ...
    private MessageHandler getServiceCallIssueEventHandler(IssueCreationService issueCreationService) {
        IssueService issueService = mockIssueService(issueCreationService);
        MeteringService meteringService = mockMeteringService();
        ServiceCallMessageHandlerFactory handlerFactory = getInjector().getInstance(ServiceCallMessageHandlerFactory.class);
