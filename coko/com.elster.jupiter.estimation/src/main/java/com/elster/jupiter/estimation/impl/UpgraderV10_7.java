...
public class UpgraderV10_7 implements Upgrader {
    ...
    private void deleteOldDestination() {
        Optional<DestinationSpec> destinationSpec = messageService.getDestinationSpec(InstallerImpl.DESTINATION_NAME);
        destinationSpec.ifPresent(destination -> {
            destination.unSubscribe(InstallerImpl.DESTINATION_NAME);
            destination.delete();
        });
    }
}
