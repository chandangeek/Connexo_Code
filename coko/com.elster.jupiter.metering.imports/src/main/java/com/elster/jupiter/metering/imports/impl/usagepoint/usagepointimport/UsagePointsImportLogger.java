...
import com.elster.jupiter.metering.imports.impl.usagepoint.FileImportLoggerImpl;
import com.elster.jupiter.metering.imports.impl.usagepoint.FileImportRecord;
import com.elster.jupiter.metering.imports.impl.usagepoint.Translations;
...
public class UsagePointsImportLogger extends FileImportLoggerImpl<FileImportRecord> {
    ...
    protected void summarizeFailedImport() {
        if (linesWithError != 0 && linesWithWarnings == 0) {
            ...
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_FAIL_WITH_ERRORS).format(linesProcessed, linesWithError));
        } else if (linesWithError != 0 && linesWithWarnings != 0) {
            // Some devices were processed with errors and warnings
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_FAIL_WITH_WARN_AND_ERRORS).format(linesProcessed, linesWithWarnings, linesWithError));
        } else if (linesWithError == 0 && linesWithWarnings != 0) {
            // Some devices were processed with warnings
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_FAIL_WITH_WARN).format(linesProcessed, linesWithWarnings));
        } else if (linesProcessed != 0 && linesWithError == 0 && linesWithWarnings == 0) {
            // Some devices were processed
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_FAIL).format(linesProcessed));
        } else if (linesProcessed == 0 && linesWithError == 0 && linesWithWarnings == 0) {
            // No devices were processed (Bad column headers)
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_NO_USAGEPOINTS_WERE_PROCESSED).format());
        }
    }
    ...
    protected void summarizeSuccessImport() {
        if (linesProcessed == 0 && linesWithError == 0) {
            ...
            fileImportOccurrence.markSuccess(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_SUCCESS).format(linesProcessed));
        } else if (linesWithError != 0 && linesWithWarnings == 0) {
            // All devices were processed but some of the devices failed
            fileImportOccurrence.markSuccessWithFailures(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_SUCCESS_WITH_ERRORS).format(linesProcessed, linesWithError));
        } else if (linesWithError != 0 && linesWithWarnings != 0) {
            ...
            fileImportOccurrence.markSuccess(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_SUCCESS_WITH_WARN).format(linesProcessed, linesWithWarnings));
        } else {
            // Fallback case
            fileImportOccurrence.markFailure(this.context.getThesaurus().getFormat(Translations.Labels.IMPORT_RESULT_NO_USAGEPOINTS_WERE_PROCESSED).format());
        }
