...
class UnitConversionNode implements ServerExpressionNode {
    ...
    UnitConversionNode(ServerExpressionNode expressionNode, Dimension dimension, VirtualReadingType targetReadingType) {
        ...
        this.sourceReadingType = VirtualReadingType.from(candidate.getIntervalLength(), dimension, candidate.getAccumulation(), candidate.getCommodity()).withMetricMultiplier(candidate.getUnitMultiplier());
