...
class ReadingTypeGeneratorForElectricity extends AbstractReadingTypeGenerator {
    ...
    private enum ReadingTypesForDemands implements ReadingTypeTemplate {
        ...
        DAILY_ACTIVE_ENERGY_MAX_DEMAND("Active energy max demand", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.DAILY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.FORWARD).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.WATT)),
        MONTHLY_ACTIVE_ENERGY_MAX_DEMAND("Active energy max demand", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.MONTHLY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.FORWARD).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.WATT)),
        ...
        DAILY_ACTIVE_ENERGY_MAX_SUPPLY("Active energy max supply", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.DAILY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.REVERSE).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.WATT)),
        MONTHLY_ACTIVE_ENERGY_MAX_SUPPLY("Active energy max supply", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.MONTHLY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.REVERSE).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.WATT)),
        ...
        MONTHLY_REACTIVE_ENERGY_MAX_DEMAND("Reactive energy max demand", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.MONTHLY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.LAGGING).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.VOLTAMPEREREACTIVE)),
        ...
        MONTHLY_REACTIVE_ENERGY_MAX_SUPPLY("Reactive energy max supply", ReadingTypeCodeBuilder.of(Commodity.ELECTRICITY_SECONDARY_METERED).period(MacroPeriod.MONTHLY).aggregate(Aggregate.MAXIMUM).accumulate(Accumulation.LATCHINGQUANTITY).flow(FlowDirection.LEADING).measure(MeasurementKind.DEMAND).in(ReadingTypeUnit.VOLTAMPEREREACTIVE)),
