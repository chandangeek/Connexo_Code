 ...
 * and returns a SQL construct (as String) that provides the modification timestamp for the visited
 * {@link ExpressionNode} or <code>0</code> if the ExpressionNode cannot provide such a timestamp.
 * A {@link NumericalConstantNode} is a good example of that.
 *
 * @author Rudi Vankeirsbilck (rudi)
 * @since 2017-04-04 (15:58)
 */
class RecordTimeFromExpressionNode implements ServerExpressionNode.Visitor<Void> {
    ...
    public Void visitProperty(CustomPropertyNode property) {
        ...
        if (property.getCustomPropertySet().isVersioned()) {
            newValue = new CustomProperty(property.sqlName() + ".modtime");
        } else {
            newValue = new Null();
        }
    ...
    public Void visitSyntheticLoadProfile(SyntheticLoadProfilePropertyNode slp) {
        this.value = this.value.accept(new TimeSeries(slp.sqlName() + "." + SqlConstants.TimeSeriesColumnNames.RECORDTIME.sqlName()));
        return null;
    }
    ...
    public Void visitVirtualDeliverable(VirtualDeliverableNode deliverable) {
        this.value = this.value.accept(new TimeSeries(deliverable.sqlName() + "." + SqlConstants.TimeSeriesColumnNames.RECORDTIME.sqlName()));
        return null;
    }

    @Override
    public Void visitVirtualRequirement(VirtualRequirementNode requirement) {
        this.value = this.value.accept(new TimeSeries(requirement.sqlName() + "." + SqlConstants.TimeSeriesColumnNames.RECORDTIME.sqlName()));
        return null;
    }
