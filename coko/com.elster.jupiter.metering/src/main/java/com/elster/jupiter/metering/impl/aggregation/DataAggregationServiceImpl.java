...
public class DataAggregationServiceImpl implements DataAggregationService {
    ...
    public CalculatedMetrologyContractData calculate(UsagePoint usagePoint, MetrologyContract contract, Range<Instant> period) {
        ...
        this.getMeterActivationSets(usagePoint, clippedPeriod).forEach(set -> this.prepare(set, contract, clippedPeriod, this.virtualFactory, deliverablesPerMeterActivation));
        try {
    ...
    private void prepare(MeterActivationSet meterActivationSet, MetrologyContract contract, Range<Instant> period, VirtualFactory virtualFactory, Map<MeterActivationSet, List<ReadingTypeDeliverableForMeterActivationSet>> deliverablesPerMeterActivation) {
        ...
        contract.getDeliverables().stream().forEach(deliverable -> this.prepare(meterActivationSet, deliverable, period, virtualFactory, deliverablesPerMeterActivation));
    }
     ...
     */
    private void prepare(MeterActivationSet meterActivationSet, ReadingTypeDeliverable deliverable, Range<Instant> period, VirtualFactory virtualFactory, Map<MeterActivationSet, List<ReadingTypeDeliverableForMeterActivationSet>> deliverablesPerMeterActivation) {
        ServerExpressionNode copied = this.copy(deliverable, meterActivationSet, virtualFactory, deliverablesPerMeterActivation, deliverable.getFormula().getMode());
     ...
     */
    private ServerExpressionNode copy(ReadingTypeDeliverable deliverable, MeterActivationSet meterActivationSet, VirtualFactory virtualFactory, Map<MeterActivationSet, List<ReadingTypeDeliverableForMeterActivationSet>> deliverablesPerMeterActivationSet, Formula.Mode mode) {
