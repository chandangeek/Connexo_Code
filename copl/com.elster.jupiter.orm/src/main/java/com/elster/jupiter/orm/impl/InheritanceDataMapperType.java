...
public class InheritanceDataMapperType<T> extends DataMapperType<T> {
    ...
    public void validate() {
        getTable().getDiscriminator()
                .orElseThrow(() -> new IllegalTableMappingException("Table " + getTable().getName() + " maps a hierarchy, but does not have a discriminator column"));
    }
    ...
    Condition condition(Class<? extends T> api) {
        ...
        if (needsRestriction(api)) {
            for (Map.Entry<String, Class<? extends T>> entry : implementations.entrySet()) {
                if (api.isAssignableFrom(entry.getValue())) {
                    result = result.and(Where.where(Column.TYPEFIELDNAME).isEqualTo(entry.getKey()));
                }
            }
        }
        return result;
