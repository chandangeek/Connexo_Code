...
class TranslationCsvEntry {
    ...
    static Optional<TranslationCsvEntry> parseFrom(String csvLine, Locale locale, String separator) {
        ...
        // Assuming the csv line contains: COMPONENT;LAYER;TRANSLATION KEY;SPECIFIC LANGUAGE TRANSLATION
        String[] values = csvLine.split(separator, 4);
        if(validateValues(values)) {
            extractedEntry = Optional.of(new TranslationCsvEntry(locale, values[0], values[1], values[2], values[3]));
        }
    ...
    private static boolean validateValues(String[] values) {
        return (values.length >= 4 && !values[0].isEmpty() && !values[1].isEmpty() && !values[2].isEmpty());
    }
