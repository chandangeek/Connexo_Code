                        ...
                        renderer: function (value) {
                            if(value.type === 'com.energyict.mdc.device.Device') {
                                if(Uni.util.Application.getAppName() === 'MultiSense') {
                                    return '<a href="' + this.router.getRoute('devices/device').buildUrl({mRID:   Ext.String.htmlEncode(Ext.String.htmlEncode(value.key))}) + '">' + Ext.String.htmlEncode(value.key) + '</a>';
                                } else {
                                    return value.key;
                                }
                            } else if (value.type === 'com.elster.jupiter.metering.UsagePoint') {
                                if(Uni.util.Application.getAppName() === 'MdmApp') {
                                    return '<a href="' + this.router.getRoute('usagepoints/view').buildUrl({mRID:  Ext.String.htmlEncode(Ext.String.htmlEncode(value.key))}) + '">' + Ext.String.htmlEncode(value.key) + '</a>';
                                } else {
                                    return value.key;
                                }
                            } else {
